...
if( $ID > 0 ){
qQuery( "update {$_ENV['eDesDictionary']}gs_op set status='{$STATUS}', cd_gs_user='{$_User}', dt_status='{$Hoy}' where cd_gs_op='{$ID}'" );
$zp = gzopen( "../_d_/log/opstatus.{$_User}", "w9" );
gzwrite( $zp, date('Y-m-d H:i:s').'|T|'.$ID.'|'.$STATUS."\n" );
gzclose($zp);
$CDI = date('Y-m-d H:i:s');
qQuery( "insert into {$_ENV['eDesDictionary']}gs_op_status values( '{$ID}', '{$CDI}', '{$STATUS}', '{$_User}' )" );
?>
<script type="text/javascript">
...
if( $USUARIO=='' ) $Hoy = '';
if( $ID > 0 ){
qQuery( "update {$_ENV['eDesDictionary']}gs_op set cd_gs_user='{$USUARIO}', dt_status='{$Hoy}' where cd_gs_op='{$ID}'" );
?>
<script type="text/javascript">
S("body").tip(eLng(27),1);
</script>
...
qQuery( "update {$_ENV['eDesDictionary']}gs_op set status='9' where status='8' and dt_status<'{$HaceUnMes}'" );
eLink('status');
echo '<style>';
if( SESS::$_D_=="" ) echo '#SelEstado TD { cursor:var(--cAuto); }';
...
<BODY onload=AjustaCeldas() onhelp='return false' oncontextmenu="return false"<?=(( SESS::$_D_!='~')?' onselectstart="return false"':'')?>>
...
<?PHP  if( SESS::$_Development && SESS::$_D_<>"" ){ ?>
...
<col class=Option>
<?PHP
echo '<col class=MasDatos'.( (SESS::$_D_!='~') ? ' style="display:none"' : '' ).'>';
echo '<col class=MasDatos'.( (SESS::$_D_!='~') ? ' style="display:none"' : '' ).'>';
...
qQuery( "select cd_gs_user, user_name, user_surname from {$_ENV['eDesDictionary']}gs_user where cd_gs_user in ( select distinct cd_gs_user from {$_ENV['eDesDictionary']}gs_op where cd_gs_user>0 )" );
while( $r=qRow() ){
$n = qCount("{$_ENV['eDesDictionary']}gs_op", "cd_gs_user='{$r[0]}'", $p2);
$_DimUser[] = array( $r[0], trim($r[1]).' '.trim($r[2]), $n );
$_DimNomUser['u'.$r[0]] = trim($r[1]).' '.trim($r[2]);
}
}
$Dim = array();
$nFalta = qCount("{$_ENV['eDesDictionary']}gs_op", "( cd_gs_user='0' or cd_gs_user='' or cd_gs_user is null )", $p2 );
...
<?=(( SESS::$_D_<>"" ) ? ' onclick="SetEstado()"' : ' style="cursor:var(--cAuto)"')?> onmouseleave='this.style.display="none"'>
<COL width=20>
