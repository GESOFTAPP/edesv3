...
function Ini(){
DGI('tableDirectorios').style.height = document.body.clientHeight-2<?=((SESS::$_D_=='~')?-30:'')?>;
}
</script>
</head>
<body onload=Ini()>
...
<?PHP
qQuery( "select A.*,T.container from {$_ENV['eDesDictionary']}gs_structure A, {$_ENV['eDesDictionary']}gs_tstructure T where A.cd_gs_structure_parent=0 and A.cd_gs_tstructure=T.cd_gs_tstructure order by T.sort,A.nm_gs_structure" );
while( $r=qArray() ){
