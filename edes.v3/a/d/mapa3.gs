...
if( $ID > 0 ){
qQuery( "update {$_ENV['eDesDictionary']}gs_op set status='{$STATUS}', cd_gs_user='{$_User}', dt_status='{$Hoy}' where id='{$ID}'" );
...
if( $USUARIO=='' ) $Hoy = '';
if( $ID > 0 ){
qQuery( "update {$_ENV['eDesDictionary']}gs_op set cd_gs_user='{$USUARIO}', dt_status='{$Hoy}' where id='{$ID}'" );
?>
<script type="text/javascript">
S("body").tip(top.eLng(27),1);
</script>
...
<BODY onload=AjustaCeldas() onhelp='return false' oncontextmenu="<?=(( SESS::$_D_!='~')?'return false;':'')?>"<?=(( SESS::$_D_!='~')?' onselectstart="return false"':'')?>>
