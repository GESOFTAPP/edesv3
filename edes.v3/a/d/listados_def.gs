...
$YaExiste = false;
if( $CdGsFormato==0 ){
$YaExiste = qCount("{$_ENV['eDesDictionary']}gs_formato", "cd_gs_entidad='{$CdGsEntidad}' and nm_gs_formato='{$NmGsFormato}'");
}else{
$YaExiste = qCount("{$_ENV['eDesDictionary']}gs_formato", "cd_gs_entidad='{$CdGsEntidad}' and nm_gs_formato='{$NmGsFormato}' and cd_gs_formato!='{$CdGsFormato}'");
}
...
if( $MODO=='a' && $ENTIDAD=='' ){
$_TReg = qCount("{$_ENV['eDesDictionary']}gs_entidad", "");
if( $_TReg == 0 ){
eMessage('NO HAY NINGUNA ENTIDAD CREADA','HES');
}else if( $_TReg == 1 ){
qSelect("{$_ENV['eDesDictionary']}gs_entidad", 'cd_gs_entidad', '');
$row = qArray();
...
qQuery( "select export_level from {$_ENV['eDesDictionary']}gs_user where cd_gs_user={$_User}" );
$row = qArray();
...
qSelect("{$_ENV['eDesDictionary']}gs_entidad", 'nm_gs_entidad', "cd_gs_entidad='{$cd_gs_entidad}'" );
$row = qArray();
...
sql_Inserta( "{$_ENV['eDesDictionary']}gs_formato",
...
$Id = qId();
qQuery( "select formato, ordenacion from {$_ENV['eDesDictionary']}gs_formato where cd_gs_formato={$Id}" );
$r = qArray();
$Formato = trim($r['formato']);
if( mb_substr($Formato,-1)!='#' ){
$Formato = mb_substr( $Formato, 0, mb_strrpos($Formato,'#')+1 );
qQuery( "update {$_ENV['eDesDictionary']}gs_formato set formato='{$Formato}' where cd_gs_formato={$Id}" );
}
$Ordenacion = trim($r['ordenacion']);
if( mb_substr($Ordenacion,-1)!='#' ){
$Ordenacion = mb_substr( $Ordenacion, 0, mb_strrpos($Ordenacion,'#')+1 );
qQuery( "update {$_ENV['eDesDictionary']}gs_formato set ordenacion='{$Ordenacion}' where cd_gs_formato={$Id}" );
}
...
if( $cd_gs_formato=="" ){
qSelect("{$_ENV['eDesDictionary']}gs_formato", '*');
$Formato = qArray();
$cd_gs_formato = $Formato['cd_gs_formato'];
}else{
qSelect("{$_ENV['eDesDictionary']}gs_formato", '*', "cd_gs_formato='{$cd_gs_formato}'");
$Formato = qArray();
}
...
if( SESS::$_WebMaster==$_ENV['ON'] ) $updateUser = "";
$sTReg = qCount("{$_ENV['eDesDictionary']}gs_formato", "cd_gs_formato='{$cd_gs_formato}'");
sql_Modifica("{$_ENV['eDesDictionary']}gs_formato",
$updateUser.
...
break;
case 'B':
sql_Borra( "{$_ENV['eDesDictionary']}gs_formato",  "cd_gs_formato='{$cd_gs_formato}'" );
...
global $ListaCampos;
$Grupo = array();
qQuery("select cd_gs_grupo, count(*) from {$_ENV['eDesDictionary']}gs_campo where cd_gs_entidad='{$cd_gs_entidad}' and extraccion='{$_ENV['ON']}' and nivel>=0 and (nivel<={$_export_level_} or nivel is null) and (log_only<>'{$_ENV['ON']}' or log_only is null) group by cd_gs_grupo");
while( $row = qRow() ){
...
$TCampos = 0;
$TGrupos = 0;
qSelect("{$_ENV['eDesDictionary']}gs_campo as C, {$_ENV['eDesDictionary']}gs_grupo as G", 'C.*,G.orden as g_orden,G.nm_gs_grupo as g_nm_grupo', "C.cd_gs_entidad='{$cd_gs_entidad}' and extraccion='{$_ENV['ON']}' and nivel>=0 and (C.nivel<={$_export_level_} or C.nivel is null) and (log_only<>'{$_ENV['ON']}' or log_only is null) and C.cd_gs_grupo=G.cd_gs_grupo", 'g_orden,g_nm_grupo,C.orden');
while( $row=qArray() ){
$TCampos++;
...
$nLineas = 0;
$DimGrupo = array();
qQuery("select cd_gs_grupo,nm_gs_grupo from {$_ENV['eDesDictionary']}gs_grupo order by 1");
...
qQuery( "select * from {$_ENV['eDesDictionary']}gs_campo where cd_gs_campo={$DimCampos[$n]}" );
$DimCampo = qArray();
...
eHTML();
?>
<LINK REL="stylesheet" HREF="<?=SESS::$_PathCSS?>/listados_def.css" TYPE="text/css">
<style>
.Transparente {
...
<?PHP
qCount("{$_ENV['eDesDictionary']}gs_campo", "cd_gs_entidad='{$cd_gs_entidad}' and extraccion='{$_ENV['ON']}' and nivel<={$_export_level_}");
if( $_TReg < 1 ){
...
qQuery("select cd_gs_user, user_name, user_surname from {$_ENV['eDesDictionary']}gs_user where permission='S' order by user_name, user_surname");
...
qQuery("select * from {$_ENV['eDesDictionary']}gs_position order by nm_gs_position");
...
qQuery("select cd_gs_node, nm_gs_node from {$_ENV['eDesDictionary']}gs_node order by nm_gs_node");
...
qQuery("select cd_gs_tree, nm_gs_tree from {$_ENV['eDesDictionary']}gs_tree order by nm_gs_tree");
