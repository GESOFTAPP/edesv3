...
eInclude($_Sql);
qQuery( "select * from {$_ENV['eDesDictionary']}gs_exp_file where cdi='{$cdi}' and cd_gs_user='{$user}'" );
$e = qArray();
...
echo '<br><br>';
qQuery( "select * from {$_ENV['eDesDictionary']}gs_formato where cd_gs_formato='{$e['cd_gs_formato']}'" );
$f = qArray();
...
echo '<u>CAMPOS:</u><br>';
if( trim($f['formato'])!='' ){
qQuery( "select * from {$_ENV['eDesDictionary']}gs_campo where cd_gs_campo in ( {$f['formato']} )" );
while( $c = qArray() ){
