...
eInclude($_Sql);
qQuery( "select cd_gs_tree from {$_ENV['eDesDictionary']}gs_user where cd_gs_user={$_User}" );
...
switch( $Estado ){
case 'A':
qQuery( "insert into {$_ENV['eDesDictionary']}gs_permission_op values ( {$_User}, {$op}, '{$_ENV['OFF']}' )" );
break;
case 'P':
qQuery( "insert into {$_ENV['eDesDictionary']}gs_permission_op values ( {$_User}, {$op}, '{$_ENV['ON']}'  )" );
break;
case 'B':
qQuery( "delete from {$_ENV['eDesDictionary']}gs_permission_op where cd_gs_user={$_User} and option_id={$op}" );
break;
default:
exit;
}
...
$TTree = 0;
qQuery( "select nm_gs_tree, description from {$_ENV['eDesDictionary']}gs_tree where cd_tree='{$cd_tree}' order by cd_gs_tree" );
while( $r = qRow() ){
$TTree++;
}
if( $TTree == 0 || qCount("{$_ENV['eDesDictionary']}gs_op", "")==0 ){
eInclude('message');
...
qQuery( "select * from {$_ENV['eDesDictionary']}gs_op where cd_tree='{$cd_tree}' and mb_substr(subtree_opt,{$TreePersonal},1)='1' order by seq" );
}else{
qQuery( "select * from {$_ENV['eDesDictionary']}gs_op where cd_tree='{$cd_tree}' and subtree_opt[{$TreePersonal},{$TreePersonal}]='1' order by seq" );
}
while( $r=qArray() ) $uTree[$r['id']] = 1;
$uOpcion = array();
qQuery( "select * from {$_ENV['eDesDictionary']}gs_permission_op where cd_gs_user={$_User} order by option_id" );
while( $r=qArray() ){
$uOpcion[$r['option_id']] = ( (trim($r['visible'])=='') ? 'N' : trim($r['visible']) );
}
if( eSqlType('mysql,mysqli') ){
qQuery( "select * from {$_ENV['eDesDictionary']}gs_op where cd_tree='{$cd_tree}' and mb_substr(subtree_opt,1,1)='1' order by seq" );
}else{
qQuery( "select * from {$_ENV['eDesDictionary']}gs_op where cd_tree='{$cd_tree}' and subtree_opt[1,1]='1' order by seq" );
}
$c = 0;
while( $r=qArray() ){
...
UnloadTabla( "{$_ENV['eDesDictionary']}gs_tree", $TXT_generation_path.'_gs_tree_.txt' );
UnloadTabla( "{$_ENV['eDesDictionary']}gs_op"  , $TXT_generation_path.'_gs_op_.txt' );
$qry = "select * from {$_ENV['eDesDictionary']}gs_op where cd_tree='{$cd_tree}' order by seq";
qQuery($qry);
while( $r=qArray() ){
$ama[]=array('tipo'=>$r['type'],'indent'=>$r['indent'],'texto'=>$r['caption'],'tip'=>$r['tip'],'id'=>$r['id'],'surl'=>$r['script_url'],'subtree_opt'=>$r['subtree_opt']);
}
$indice=0;
$qry="select * from {$_ENV['eDesDictionary']}gs_tree where cd_tree='{$cd_tree}' order by cd_gs_tree";
qQuery($qry,$p1);
while( $r=qArray($p1) ){
$cod=$r['cd_gs_tree'];
