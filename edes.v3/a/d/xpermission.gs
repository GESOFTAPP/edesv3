...
if( qCount("{$_ENV['eDesDictionary']}gs_tpermission", 'cd_gs_tpermission='.$_POST['cd_gs_tpermission'])>0 ){
qQuery("update {$_ENV['eDesDictionary']}gs_tpermission set note='".trim(str_replace('<BR>',"\n",$_POST['note']))."' where cd_gs_tpermission=".$_POST['cd_gs_tpermission']);
$ZIP = false;
$file = "gs_tpermission";
$sTABLA = "{$_ENV['eDesDictionary']}gs_tpermission";
$pnt = '';
...
qQuery("insert into {$_ENV['eDesDictionary']}gs_activity (cd_gs_user, cdi, script, edes, email) values ({$_User}, '".date('Y-m-d H:i:s')."', '/tree/{$file}.unl', 'C', '".SESS::$_UserEMail."')");
}
}
...
qQuery( "select * from {$_ENV['eDesDictionary']}gs_{$RolPermission}permission order by cd_gs_tpermission", $p1 );
while( $r2=qArray($p1) ){
...
$TP = array();
$TPSinOp = array();
qQuery("select * from {$_ENV['eDesDictionary']}gs_tpermission where active='{$_ENV['ON']}' order by type,note");
while( $r=qArray() ){
...
$Folder = ',';
qQuery("select cd_gs_op,indent,type,caption,seq from {$_ENV['eDesDictionary']}gs_op where cd_gs_op in ({$Opciones}) order by seq");
while( $r=qArray() ){
if( $r['type']=='O' ){
$i = $r['indent'];
$s = $r['seq'];
...
qQuery("select cd_gs_op,indent,caption,type from {$_ENV['eDesDictionary']}gs_op where seq<{$s} and indent={$i} and type='F' order by seq desc", $p1);
$f = qArray($p1);
...
if( SESS::$_Development ) $xPermission .= ' title="'.str_replace('"','\"',$TP[$n]['nm_gs_tpermission']).'"';
$xPermission .= '>';
}
}
...
if( SESS::$_Development ) echo ' title="'.str_replace('"','\"',$TPSinOp[$n]['nm_gs_tpermission']).'"';
echo '>';
}
echo '</tr></table>';
echo '</center>';
