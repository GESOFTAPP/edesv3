...
qQuery( "update {$_ENV['eDesDictionary']}gs_structure set cd_gs_structure_parent='{$_GET['cPadre']}' where cd_gs_structure={$_GET['MoverDir']}" );
...
qQuery( "select * from {$_ENV['eDesDictionary']}gs_structure where cd_gs_structure_parent={$BorrarDir}", $p1 );
while( $r=qArray($p1) ){
if( $r['cd_gs_structure_parent'] > 0 ) BorrarHijos( $r['cd_gs_structure'] );
qQuery( "delete from {$_ENV['eDesDictionary']}gs_structure where cd_gs_structure={$r['cd_gs_structure']}", $p2 );
}
}
BorrarHijos( $BorrarDir );
qQuery( "delete from {$_ENV['eDesDictionary']}gs_structure where cd_gs_structure={$_GET['BorrarDir']}" );
echo '<script type="text/javascript">';
...
qQuery( "select A.*, T.container from {$_ENV['eDesDictionary']}gs_structure A, {$_ENV['eDesDictionary']}gs_tstructure T where A.cd_gs_structure_parent={$_GET['LoadDir']} and A.cd_gs_tstructure=T.cd_gs_tstructure order by T.sort,A.nm_gs_structure" );
$n=0;
while( $r=qArray() ){
$TieneHijos = ( ( qCount("{$_ENV['eDesDictionary']}gs_structure", "cd_gs_structure_parent={$r['cd_gs_structure']}", $p ) > 0 ) ? '_0' : '_1' );
$Sufijo = $TieneHijos;
...
if( $r['cd_gs_user'] > 0 ){
qQuery( "select user_name,user_surname, email from {$_ENV['eDesDictionary']}gs_user where cd_gs_user='{$r['cd_gs_user']}'", $p1 );
$nm = qRow($p1);
...
$_DEBUG = 1;
qQuery( "update {$_ENV['eDesDictionary']}gs_structure set nm_gs_structure='{$_GET['ModificarDir']}', cd_gs_tstructure={$_GET['Tipo']}, cd_gs_user='{$_GET['User']}', mfilter='{$_GET['Filter']}' where cd_gs_structure={$_GET['cElemento']}" );
?>
<script type="text/javascript">
var _WOPENER = window.frameElement.WOPENER;
...
qQuery( "insert into {$_ENV['eDesDictionary']}gs_structure (    nm_gs_structure   , cd_gs_tstructure, cd_gs_structure_parent, cdi_insert, cdi_update, mfilter )
