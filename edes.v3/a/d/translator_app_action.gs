...
$q="select * from {$_ENV['eDesDictionary']}gs_script where cd_gs_script in(
select cd_gs_script from {$_ENV['eDesDictionary']}gs_transchange where cd_gs_transchange in({$ids}) group by 1
)";
qQuery($q,$p1);
$s=array();
while($r=qArray($p1)){
qQuery("select word_id from {$_ENV['eDesDictionary']}gs_transchange where cd_gs_script={$r['cd_gs_script']} and cd_gs_transchange in({$ids})",$p2);
$wi=array();
while($rr=qArray($p2)){
$wi[] = $rr['word_id'];
}
...
qQuery("select cd_gs_script from {$_ENV['eDesDictionary']}gs_transchange where tf_script='{$_ENV['ON']}' group by cd_gs_script");
while($r=qArray()){
...
qQuery("select nm_gs_script,filepath,type from {$_ENV['eDesDictionary']}gs_script where cd_gs_script='{$cd_gs_script}'",$p1);
$r=qArray($p1);
...
qQuery("update {$_ENV['eDesDictionary']}gs_transchange set tf_script='' where cd_gs_script='{$cd_gs_script}'",$p2);
$cta_actualizados++;
echo <<<EOD
...
$tag = array();
$lng=array();
qQuery("select cd_gs_language from {$_ENV['eDesDictionary']}gs_transchange where cd_gs_script='{$cd_gs_script}' group by cd_gs_language",$p2);
while($r=qArray($p2)){
...
$t =array();
qQuery("select * from {$_ENV['eDesDictionary']}gs_transchange where cd_gs_script='{$cd_gs_script}' order by word_id,cd_gs_language",$p2);
while( $r=qArray($p2) ){
...
$cdi_changed	= date('Y-m-d H:i:s');
$now=date('Y-m-d H:i:s');
$q="update {$_ENV['eDesDictionary']}gs_transchange set word_val='{$word_val}', word_val_md5='{$word_val_md5}', cdi_changed='{$cdi_changed}', tf_changed='', tf_script='{$_ENV['ON']}', comment='{$comment}'
where cd_gs_transchange in({$ids})";
qQuery($q);
$tf_changed = ($cd_gs_language==$deflang) ? $_ENV['ON'] : $_ENV['OFF'];
qQuery("select cd_gs_script,word_id from {$_ENV['eDesDictionary']}gs_transchange where cd_gs_transchange in({$ids})",$p2);
while($r=qArray($p2)){
qQuery("update {$_ENV['eDesDictionary']}gs_transchange set comment='{$comment}', tf_changed='{$tf_changed}', tf_script='{$_ENV['ON']}'
...
$q="update {$_ENV['eDesDictionary']}gs_op_lng
...
EOD;
qQuery("select * from {$_ENV['eDesDictionary']}gs_language where cd_gs_language='{$lng}'");
$r=qArray();
$col[] = <<<EOD
...
$gs_op_lng=array();
$gs_op_deflang=array();
qQuery("select * from {$_ENV['eDesDictionary']}gs_op_lng order by md5");
while($r=qArray()){
$r['flg'] = 0;
...
$t =array();
qQuery("select md5,count(*) as cta from {$_ENV['eDesDictionary']}gs_op_lng
...
qQuery("select * from {$_ENV['eDesDictionary']}gs_transchange where type='{$GS_TRANSCHANGE_TYPE}' order by word_val_md5");
while($rr=qArray()){
$rr['flg'] = 0;
...
$t =array();
qQuery("select word_val_md5,count(*) as cta from {$_ENV['eDesDictionary']}gs_transchange
...
global $deflang;
$natratar=qCount("{$_ENV['eDesDictionary']}gs_op", "");
echo <<<EOD
<script type="text/javascript">
...
EOD;
ob_flush();
qQuery("select * from {$_ENV['eDesDictionary']}gs_language where tf_translation='{$_ENV['ON']}'");
while($r=qArray()){
$rlng[$r['cd_gs_language']] = $r;
}
$cta=0;
qQuery("select * from {$_ENV['eDesDictionary']}gs_op order by seq,seq_parent", $p1);
while($r=qArray($p1)){
...
$nr=qCount("{$_ENV['eDesDictionary']}gs_op_lng", "id='{$r['cd_gs_op']}' and cd_gs_language='{$cd_gs_language}'");
if( $nr==0 ){
$q="insert into {$_ENV['eDesDictionary']}gs_op_lng (id,cd_gs_language,caption,tip,md5) values(
'{$r['cd_gs_op']}',
'{$cd_gs_language}',
'{$r['caption']}',
'{$r['tip']}',
...
if( $cd_gs_language==$deflang ){
qQuery("select md5 from {$_ENV['eDesDictionary']}gs_op_lng where id='{$r['cd_gs_op']}' and cd_gs_language='{$cd_gs_language}'",$p2);
$rr=qArray($p2);
if( $rr['md5']!=$mmd5 ){
$q="update {$_ENV['eDesDictionary']}gs_op_lng
...
qQuery($q,$p3);
$q="update {$_ENV['eDesDictionary']}gs_op_lng set tf_changed='{$tf_changed}'	where id='{$r['cd_gs_op']}'";
qQuery($q,$p3);
}
}
}
...
global $deflang;
$gs_language=array();
qQuery("select * from {$_ENV['eDesDictionary']}gs_language order by cd_gs_language");
while($r=qArray()){
...
if( qCount("{$_ENV['eDesDictionary']}gs_script", "filepath='{$filepath}'")==0 ){
$nm_script	= basename($archivo);
$pinfo		= pathinfo($archivo);
$extension	= $pinfo['extension'];
qQuery("insert into {$_ENV['eDesDictionary']}gs_script(nm_gs_script,extension,filepath,type)
values('{$nm_script}','{$extension}','{$filepath}','{$type}')");
$cd_gs_script = qId();
}else{
qQuery("select * from {$_ENV['eDesDictionary']}gs_script where filepath='{$filepath}'");
$rs = qArray();
...
if( qCount("{$_ENV['eDesDictionary']}gs_language", "cd_gs_language='{$cd_gs_language}'")==0 ){
...
$word_val		= urlencode(trim($word_val));
$word_val_md5	= md5($word_val);
$nr = qCount("{$_ENV['eDesDictionary']}gs_transchange", "cd_gs_script={$cd_gs_script} and cd_gs_language='{$cd_gs_language}' and word_id='{$word_id}' ");
if( $nr==0 ){
llog( str_replace('"',"\"","(N){$cd_gs_language} - {$word_id} - {$word_val}" ) );
$now = date('Y-m-d H:i:s');
$q = "insert into {$_ENV['eDesDictionary']}gs_transchange (cd_gs_script,cd_gs_language,word_id,word_val,word_val_md5,cdi_add,comment,type,word_val_old,word_val_md5_old)
...
qQuery("select * from {$_ENV['eDesDictionary']}gs_transchange where cd_gs_script={$cd_gs_script} and cd_gs_language='{$cd_gs_language}' and word_id='{$word_id}' ");
$rt = qArray();
if( $word_val_md5 != $rt['word_val_md5'] ){
$q="update {$_ENV['eDesDictionary']}gs_transchange set tf_changed='{$_ENV['ON']}', word_val='{$word_val}', word_val_md5='{$word_val_md5}', word_val_old='{$word_val}', word_val_md5_old='{$word_val_md5}', comment='{$comment}'  where cd_gs_script={$cd_gs_script} and cd_gs_language='{$cd_gs_language}' and word_id='{$word_id}'";
llog($q);
qQuery($q);
$q="update {$_ENV['eDesDictionary']}gs_transchange set tf_changed='{$_ENV['ON']}' where cd_gs_script={$cd_gs_script} and word_id='{$word_id}'";
qQuery($q);
}
}
}
...
if( $nr==0 ){
$now = date('Y-m-d H:i:s');
$q = "insert into {$_ENV['eDesDictionary']}gs_transchange(cd_gs_script,cd_gs_language,word_id,word_val,word_val_md5,cdi_add,comment,type,word_val_old,word_val_md5_old,tf_changed)
...
qQuery("update {$_ENV['eDesDictionary']}gs_transchange set tf_changed='{$_ENV['ON']}', word_val='{$word_val}', word_val_md5='{$word_val_md5}', word_val_old='{$word_val}', word_val_md5_old='{$word_val_md5}', comment='{$comment}'  where cd_gs_script={$cd_gs_script} and cd_gs_language='{$cd_gs_language}' and word_id='{$word_id}'");
qQuery("update {$_ENV['eDesDictionary']}gs_transchange set tf_changed='{$_ENV['ON']}' where cd_gs_script={$cd_gs_script} and word_id='{$word_id}'");
...
qQuery("update {$_ENV['eDesDictionary']}gs_transchange set tf_changed='', word_val_old='{$rt['word_val']}', word_val_md5_old='{$rt['word_val_md5']}' where cd_gs_transchange={$rt['cd_gs_transchange']}",$p5);
}
}
}
}
...
if( count($w) ){
$aborrar=array();
qQuery("select cd_gs_transchange,cd_gs_language,word_id from {$_ENV['eDesDictionary']}gs_transchange where cd_gs_script={$cd_gs_script} and word_id not in (". implode(',',$w) .")",$p2);
while($rt=qArray($p2)){
...
if( count($aborrar) ){
qQuery("delete from {$_ENV['eDesDictionary']}gs_transchange where cd_gs_transchange in(".implode(',',$aborrar).")");
}
}
