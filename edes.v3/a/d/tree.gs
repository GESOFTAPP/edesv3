...
qQuery( "update {$_ENV['eDesDictionary']}gs_op set subtree_opt='{$subtree_opt}' where id='{$id}' and cd_tree='{$cd_tree}'" );
}
if( $CONARBOLES==1 ){
make_Txt( $cd_tree );
...
$TTree = 0;
qQuery( "select nm_gs_tree, description from {$_ENV['eDesDictionary']}gs_tree order by cd_gs_tree" );
while( $r = qRow() ){
$TTree++;
...
$OpEn = array();
qQuery( "select cd_gs_op,cd_gs_tree from {$_ENV['eDesDictionary']}gs_tree_op" );
while( $r=qArray() ){
$OpEn[$r[0]][$r[1]] = 1;
}
$OpNo = "'U'";
if( SESS::$_Development ) $OpNo .= "";
else if( $_Test ) $OpNo .= ",'D'";
else $OpNo .= ",'D','T'";
if( $_PrintToPDF ){ $usuCursor = array(); $nr=0; }
qQuery( "select * from {$_ENV['eDesDictionary']}gs_op where show_type not in ({$OpNo}) order by seq" );
$c = 0;
while( $r=qArray() ){
