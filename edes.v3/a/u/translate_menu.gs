...
while( $r=qArray($p) ){
$txt = $NewValor;
$Sql = "insert into {$_ENV['eDesDictionary']}gs_op_lng values ( ".$r['cd_gs_op'].", '{$lng}', '".$r['caption_tip']."', '".$txt."', '{$OldMD5}', '' )";
eTrace( $Sql );
qQuery( $Sql );
}
}else{
...
if( $CaptionTip=='C' ){
$Sql = "update {$_ENV['eDesDictionary']}gs_op set caption=(select caption from {$_ENV['eDesDictionary']}gs_op_lng where cd_gs_op={$_ENV['eDesDictionary']}gs_op.cd_gs_op and cd_gs_language='{$Idioma}' and {$MasModificar} ) where cd_gs_op in ( {$xPK} )";
}else{
$Sql = "update {$_ENV['eDesDictionary']}gs_op set tip=(select caption from {$_ENV['eDesDictionary']}gs_op_lng where cd_gs_op={$_ENV['eDesDictionary']}gs_op.cd_gs_op and cd_gs_language='{$Idioma}' and {$MasModificar} ) where cd_gs_op in ( {$xPK} )";
}
eTrace( $Sql );
qQuery( $Sql);
}
