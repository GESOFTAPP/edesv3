...
file_put_contents($File, S::$_User.','.(((int)eGetMicroTime())+120).','.SESS::$_UserName.','.SESS::$_UserEMail);
}
...
qQuery("update {$_ENV['eDesDictionary']}gs_op set show_type='{$tmp[2]}' where cd_gs_op='{$tmp[1]}'");
}
}
@unlink($File);
}
...
$Dim = array();
$nMax = 0;
qQuery("select cd_gs_user,email,count(*) from {$_ENV['eDesDictionary']}gs_activity where script='{$Buscar}' group by 1,2 order by 3 desc");
while( $r=qRow() ){
qQuery("select max(cdi) from {$_ENV['eDesDictionary']}gs_activity where script='{$Buscar}' and cd_gs_user={$r[0]}", $p1);
list($cdi) = qRow($p1);
$Dim[] = $cdi.'|'.$r[2].'|'.$r[1];
...
$MaxId = 1;
$nReg = qCount("{$_ENV['eDesDictionary']}gs_op");
if( $nReg>0 ){
qQuery("select max(cd_gs_op) from {$_ENV['eDesDictionary']}gs_op order by 1 desc");
list($MaxId) = qRow();
...
$txt .= ",'{$Hoy}'";
$txt .= ",'','{$Hoy}'";
$sql = "insert into {$_ENV['eDesDictionary']}gs_op (seq,cd_gs_op,show_type,mode,icon,indent,type,caption,script_url,tip,icons,alias, dt_add, status,dt_status) values ({$txt})";
...
$NumDelete++;
$xID = $xID * -1;
$sql = "delete from {$_ENV['eDesDictionary']}gs_op where cd_gs_op={$xID}";
qQuery("delete from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_op={$xID}");
}else{
$sql = "update {$_ENV['eDesDictionary']}gs_op set seq='{$Seq}', show_type='{$tmp[1]}', mode='{$tmp[2]}', icon='{$tmp[3]}', indent='{$tmp[4]}', type='{$tmp[5]}',
caption='{$tmp[6]}',
script_url='{$tmp[7]}',
tip='{$tmp[8]}',
icons='{$tmp[9]}',
...
qQuery("update {$_ENV['eDesDictionary']}gs_op set show_type='' where type<>'O' and show_type<>''");
if( $Test ){
echo '</table>';
echo $_POST['grabar'];
}else{
...
closedir($uDir);
$ArbolTxt = '';
qQuery("select * from {$_ENV['eDesDictionary']}gs_op order by seq");
while( $r=qRow() ){
...
qQuery("insert into {$_ENV['eDesDictionary']}gs_activity (cd_gs_user, cdi, script, edes, email) values ({$_User}, '". date('Y-m-d H:i:s')."', '/tree/gs_op.unl', 'C', '".SESS::$_UserEMail."')");
}
?>
<script type="text/javascript">
...
qQuery("update {$_ENV['eDesDictionary']}gs_tree set permission='".(($_GET['TreeOnOff']==$_ENV['ON']) ? $_ENV['ON'] : $_ENV['OFF'])."' where cd_gs_tree=".$_GET['TreeCd']);
?>
<script type="text/javascript">
...
$_POST['NmTree'] = str_replace( "'", '&#39;', $_POST['NmTree'] );
qQuery("insert into {$_ENV['eDesDictionary']}gs_tree (nm_gs_tree, filename) values ('".$_POST['NmTree']."', '')");
$id = qId();
?>
<script type="text/javascript">
...
$txt = '';
qQuery("select O.seq, T.cd_gs_op from {$_ENV['eDesDictionary']}gs_tree_op T left join {$_ENV['eDesDictionary']}gs_op O on T.cd_gs_op=O.cd_gs_op where T.cd_gs_tree=".$_POST['LoadTree'].' order by O.seq desc');
while( $r=qRow() ){
...
<?PHP
if( $_POST['IgualQue']>0 ){
qQuery("insert into {$_ENV['eDesDictionary']}gs_tree_op (select {$id}, cd_gs_op from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_tree={$_POST['IgualQue']})");
}
eEnd();
}
...
if( $Test ){
eTrace('Tree: '.$Dim[0]);
eTrace("update {$_ENV['eDesDictionary']}gs_tree set nm_gs_tree='".$NomTree."' where cd_gs_tree=".$Tree);
echo '<table border=1>';
}else{
_Log('Comprueba que el nombre no se duplique');
if( qCount("{$_ENV['eDesDictionary']}gs_tree", "nm_gs_tree='".$NomTree."' and cd_gs_tree<>".$Tree)>0 ){
qQuery("select nm_gs_tree from {$_ENV['eDesDictionary']}gs_tree where cd_gs_tree=".$Tree);
list($NomTree) = qRow();
?>
<script type="text/javascript">
...
qQuery("update {$_ENV['eDesDictionary']}gs_tree set nm_gs_tree='".$NomTree."', cdi='".date('Y-m-d H:i:s')."' where cd_gs_tree=".$Tree);
...
qQuery("delete from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_tree={$Tree}");
}
...
if( trim($Dim[$n])!='' ) qQuery("insert into {$_ENV['eDesDictionary']}gs_tree_op values ({$Tree}, {$Dim[$n]})");
}
$TReg--;
M($TReg);
$Contenido = ob_get_contents();
...
$DimIdiomas = array();
qQuery("select cd_gs_language from {$_ENV['eDesDictionary']}gs_language");
while( $r=qRow() ) $DimIdiomas[] = trim($r[0]);
if( count($DimIdiomas)==0 ) $DimIdiomas[] = 'es';
$CdTree = $Tree;
_Log("Obtiene el nombre del arbol");
qQuery("select filename from {$_ENV['eDesDictionary']}gs_tree where cd_gs_tree={$CdTree}", $na);
...
$DimLabel = array();
qQuery("select cd_gs_op,caption,caption_tip from {$_ENV['eDesDictionary']}gs_op_lng where cd_gs_language='{$Lenguaje}' order by cd_gs_op");
while( $r=qRow() ) $DimLabel[trim($r[2]).$r[0]] = Decode(trim($r[1]));
$txt = '';
qQuery("select * from {$_ENV['eDesDictionary']}gs_op where cd_gs_op in (select cd_gs_op from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_tree={$CdTree}) order by seq");
while( $r=qArray() ){
...
if( SESS::$_LanguageDefault==$Lenguaje ){
...
$Test = false;
$DimAdd = array();
qQuery("select cd_gs_op, dt_add from {$_ENV['eDesDictionary']}gs_op");
...
$sql = "insert into {$_ENV['eDesDictionary']}gs_op values ($txt)";
...
eTrace("delete from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_tree={$NArbol}");
echo '<table border=1>';
}else{
qQuery("delete from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_tree={$NArbol}");
}
for( $n=2; $n<count($Dim); $n++ ){
if( $Test ){
echo '<tr><td>'.$Dim[0].'<td>'.$Dim[$n];
echo '<td>'."insert into {$_ENV['eDesDictionary']}gs_tree_op values({$NArbol}, {$Dim[$n]})";
}else{
qQuery("insert into {$_ENV['eDesDictionary']}gs_tree_op values({$NArbol}, {$Dim[$n]})");
}
}
if( $Test ){
echo '</table>';
...
qQuery("select distinct(cd_gs_tree) from {$_ENV['eDesDictionary']}gs_tree where cd_gs_tree in (select cd_gs_tree from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_op={$Tree})");
$xTree = '';
...
$DimArbols = array();
qQuery("select cd_gs_tree from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_op in ({$UnaOpcion})");
while( $r=qRow() ) $DimArbols[$r[0]*1] = 1;
qQuery("delete from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_op in ({$UnaOpcion})");
if( $Arboles!='' ){
$DimIdiomas = array();
qQuery("select cd_gs_language from {$_ENV['eDesDictionary']}gs_language");
while( $r=qRow() ) $DimIdiomas[] = trim($r[0]);
...
$Opcion = $Opciones[$o];
if( qCount("{$_ENV['eDesDictionary']}gs_tree_op", "cd_gs_tree={$Arbol} and cd_gs_op={$Opcion}")==0 ){
qQuery("insert into {$_ENV['eDesDictionary']}gs_tree_op (cd_gs_tree,cd_gs_op) values ({$Arbol},{$Opcion})");
}
}
$DimArbols[$Arbol*1] = 1;
}
}
foreach( $DimArbols as $Arbol=>$v ) GeneraTreeTxt( $Arbol, $DimIdiomas, SESS::$_LanguageDefault );
?>
<script type="text/javascript">
top.S.init(window);
var Obj = window.frameElement.WOPENER
...
qQuery("select cd_gs_op from {$_ENV['eDesDictionary']}gs_op where cd_gs_op not in (select cd_gs_op from {$_ENV['eDesDictionary']}gs_tree_op)");
$Op = '';
while( $r=qRow() ) $Op .= ','.$r[0];
if( $Op!='' ) $Op .= ',';
?>
...
if( SESS::$_D_=='~' ) echo '<script>top.S.edes(window);</script>';
?>
<SCRIPT type="text/javascript">
...
<?PHP
if( !SESS::$_Development || $_GET['_GEST']!="ALL" ){
...
<?PHP
if( SESS::$_Development ) echo '_EditandoArbol = false;';
?>
_TreeON = false;
_NTree = null;
...
<?PHP
$_DimIcon = array();
qQuery("select * from {$_ENV['eDesDictionary']}gs_op_ico where global<>'{$_ENV['ON']}' or global is null order by position");
while( $r=qArray() ){
...
<?PHP } ?>
<?PHP if( $_WithDevelopment && eSubstrCount('~AMPDH',SESS::$_D_) == 1 && $_GET['_GEST']=="ALL" ){ ?>
<TH class=THOption style='cursor:var(--cPointer);'><span style='width:30px'></span>
<?PHP  if( SESS::$_Development && $_GET['_GEST']=="ALL" ){ ?>
...
<tbody>
<?PHP
if( $_GET['TREE']==1 && !SESS::$_Development ){
qQuery("select * from {$_ENV['eDesDictionary']}gs_op where show_type<>'U' and show_type<>'D' order by seq");
}else{
qQuery("select * from {$_ENV['eDesDictionary']}gs_op order by seq");
}
$c = 0;
...
<?PHP
qQuery("select * from {$_ENV['eDesDictionary']}gs_tree order by nm_gs_tree");
while( $r=qArray() ){
...
$Hoy = date('Y-m-d');
$q = "insert into {$_ENV['eDesDictionary']}gs_op (cd_gs_op, mode, seq, caption, indent, type, script_url, dt_add, icon, status, dt_status) values ({$prox}, '{$xMode}', {$carbol}, '{$narbol}', {$niv}, '{$sType}', '{$opcion}', '{$xFecha}', '{$xIcon}', '', '{$Hoy}')";
