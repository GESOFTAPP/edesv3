<?PHP
if( S::$_User==-1 || SESS::$_D_=="" || $_gsID!=getmypid() ) exit;
...
"\t".'"Tools"=>'.($tmp[6]*1)."\n".
');'."\n".
'$_gsNomUser = SESS::$_gsACCESO["EMAIL"];'."\n".
"?".">";
qQuery("select cd_gs_user from {$_ENV['eDesDictionary']}gs_user where email='{$tmp[5]}'");
list($pk) = qRow();
if( $pk>0 ){
$DimUser[] = $pk;
$file = "../_d_/".$pk.'.dvl';
...
if( eSubstrCount($txt, mb_chr(9).SESS::$_UserEMail.mb_chr(9))==0 ) ePrintR("");
return $txt;
}
