...
qQuery("select * from {$_ENV['eDesDictionary']}gs_question where cd_gs_poll=".($_POST["cd_gs_poll"]*1)." and cd_gs_question=".($_POST["cd_gs_question"]*1));
$r = qArray();
...
if( $r["orden"]==1 ){
qQuery("delete from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll='{$_POST['cd_gs_poll']}' and cd_gs_campaign='{$_POST['cd_gs_campaign']}' and cd_gs_reply_user='{$_POST['cd_gs_reply_user']}'");
qQuery("delete from {$_ENV['eDesDictionary']}gs_campaign_user where cd_gs_poll='{$_POST['cd_gs_poll']}' and cd_gs_campaign='{$_POST['cd_gs_campaign']}' and cd_gs_user='{$_POST['cd_gs_reply_user']}'");
...
qQuery("delete from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll='{$_POST['cd_gs_poll']}' and cd_gs_campaign='{$_POST['cd_gs_campaign']}' and cd_gs_reply_user='{$_POST['cd_gs_reply_user']}' and cd_gs_question='{$_POST['cd_gs_question']}'");
}
}
...
qQuery("select mapa from {$_ENV['eDesDictionary']}gs_poll_img where cd_gs_poll_img={$pkImg}");
list($mapa) = qRow();
$mapa = str_replace(
array("&#34;"),
array('"'    ),
...
qQuery("insert into {$_ENV['eDesDictionary']}gs_reply (cd_gs_poll, cd_gs_campaign, cd_gs_reply_user, cd_gs_question, segundos, respuesta, aciertos, puntos) values ('{$_POST['cd_gs_poll']}', '{$_POST['cd_gs_campaign']}', '{$_POST['cd_gs_reply_user']}', '{$_POST['cd_gs_question']}', '{$_POST['segundos']}', '{$respuesta}', {$aciertos}, {$puntos})");
$pk = qId();
...
$NO = array();
$NOVARIAS = array();
qQuery("select palabra,varias from {$_ENV['eDesDictionary']}gs_tabu order by palabra", $p1);
...
continue;
default:
qQuery("insert into {$_ENV['eDesDictionary']}gs_poll_dct (cd_gs_poll, cd_gs_campaign, cd_gs_question, cd_gs_reply, palabra, posicion) values ('{$_POST['cd_gs_poll']}', '{$_POST['cd_gs_campaign']}', '{$_POST['cd_gs_question']}', $pk, '{$palabra}', {$nPalabra})");
}
$nPalabra++;
}
}
