...
$question = $dim[$n];
qQuery("delete from {$_ENV['eDesDictionary']}gs_question where cd_gs_poll={$poll} and cd_gs_question={$question}");
}
}
$txt = $_POST["DATOS"];
$dim = $txt;
...
if( qCount("{$_ENV['eDesDictionary']}gs_question", "cd_gs_poll={$poll} and cd_gs_question={$json['cd_gs_question']}")>0 ){
qQuery("update {$_ENV['eDesDictionary']}gs_question set
...
qQuery("insert into {$_ENV['eDesDictionary']}gs_question (cd_gs_poll,cd_gs_question,orden,pregunta,respuesta_type,respuesta_con_salto,pregunta_leng,json,marcar_tr) values ({$poll}, {$json['cd_gs_question']}, {$json['orden']},'{$json['pregunta']}', '{$json['respuesta_type']}', '{$json['respuesta_con_salto']}', {$preguntaLeng}, '{$json['json']}', '{$json['marcar_tr']}')");
}
}
die("Grabado");
?>