...
qQuery("select nm_gs_poll from {$_ENV['eDesDictionary']}gs_poll where cd_gs_poll={$pkPoll}");
list($nmPoll) = qArray();
...
$dim = array();
qQuery("select orden, respuesta_type from {$_ENV['eDesDictionary']}gs_question where cd_gs_poll={$pkPoll} order by orden");
while( $r=qRow() ){
...
from {$_ENV['eDesDictionary']}gs_reply as R left join {$_ENV['eDesDictionary']}gs_question as P on R.cd_gs_poll=P.cd_gs_poll and R.cd_gs_question=P.cd_gs_question
...
$TotalRespuestas += $r[1];
qQuery("select cd_gs_question from {$_ENV['eDesDictionary']}gs_question where cd_gs_poll={$pkPoll} and orden={$nPregunta} order by orden");
...
if( $pkQuestion>0 ){
$i = 0;
qQuery("select segundos from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_campaign={$pkCampaign} and cd_gs_question={$pkQuestion} order by segundos");
while($r2=qRow()){
$val = $r2[0];
...
echo "</tfoot>";
echo "</table>";
qQuery("select sum(puntos) from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_campaign={$pkCampaign}");
$r = qRow();
if( $r[0]>0 ){
qQuery("select punt,num from (select cd_gs_reply_user, sum(puntos) punt, count(*) num from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_campaign={$pkCampaign} group by 1 order by 1) as x");
echo '<table border="0" cellspacing="1" cellpadding="3" style="display:table" class="col_0r col_1r col_2r col_3r col_4r col_5r col_6r">';
...
echo "</td></tr>";
$totalPreguntas = 0;
qQuery("select * from {$_ENV['eDesDictionary']}gs_question where cd_gs_poll={$pkPoll} order by orden", $pPregunta);
...
$nPregunta = 0;
$totalPreguntas++;
qQuery("select count(*), sum(segundos), min(segundos), max(segundos) from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign}");
...
echo "</table>";
qQuery("select respuesta, count(*) from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by respuesta order by respuesta");
while($r=qRow()){
...
$nPregunta = 0;
qQuery("select aciertos, count(*) from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by aciertos order by aciertos");
while($r=qRow()){
$nPregunta++;
if($_VerTrace)eTrace("Respuesta: ".$r[0].' = Votos: '.$r[1]);
$res[0][$r[0]+1] = $r[1];
...
$maxRespuestas = 20;
$dimResultado = array();
qQuery("select respuesta, count(*) from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by 1 order by 2 desc,1");
$n=0;
while($r=qRow()){
...
$maxRespuestas = 20;
$dimResultado = array();
qQuery("select palabra,count(*) from {$_ENV['eDesDictionary']}gs_poll_dct where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by 1 order by 2 desc,1");
$palabra = array();
$n=0;
while($r=qRow()){
if( ++$n>$maxRespuestas ) break;
...
qQuery("select palabra,count(*) from {$_ENV['eDesDictionary']}gs_poll_dct where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign}
and palabra<>'{$palabra[$p]}' and cd_gs_reply in (
select cd_gs_reply from {$_ENV['eDesDictionary']}gs_poll_dct where cd_gs_poll=8 and cd_gs_question={$pkQuestion} and palabra='{$palabra[$p]}'
...
case "MM":
$todo = array();
qQuery("select respuesta,count(*) from {$_ENV['eDesDictionary']}gs_reply where cd_gs_poll={$pkPoll} and cd_gs_question={$pkQuestion} and cd_gs_campaign={$pkCampaign} group by 1 order by 1");
while($r=qRow()) $todo[] = $r;
