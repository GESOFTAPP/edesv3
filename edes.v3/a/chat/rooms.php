...
<?=((SESS::$_Development)? 'S("#ICON_CHATSUBMIT").css("padding-right:30px;");' : "")?>;
...
<?PHP
qQuery("select max(last_msg) from {$_ENV['eDesDictionary']}gs_chat_room where cdi_close is null");
list($LastMsg) = qRow();
if( $LastMsg=="" ) $LastMsg = "2000-01-01 00:00:00";
qQuery("select cdi_insert, cd_gs_chat_room from {$_ENV['eDesDictionary']}gs_chat_msg where cd_gs_chat_msg in (select max(cd_gs_chat_msg) from {$_ENV['eDesDictionary']}gs_chat_msg where cdi_insert>'{$LastMsg}' group by cd_gs_chat_room) order by cdi_insert desc");
while($r=qRow()){
qQuery("update {$_ENV['eDesDictionary']}gs_chat_room set last_msg='{$r[0]}' where cd_gs_chat_room={$r[1]}");
}
$totalMsgSinLeer = 0;
$_DimRoom = array();
$_DimRoomON = array();
...
(select count(*) from {$_ENV['eDesDictionary']}gs_chat_msg_user where cd_gs_user=".S::$_User." and cd_gs_chat_room=gs_chat_room.cd_gs_chat_room and cdi_read is null) msg_sin_leer,
(select max(cdi_read) from {$_ENV['eDesDictionary']}gs_chat_msg_user where cd_gs_user=".S::$_User." and cd_gs_chat_room=gs_chat_room.cd_gs_chat_room) cdi_read
from {$_ENV['eDesDictionary']}gs_chat_room
where cdi_close is null and (type_room='S' or EXISTS (select cd_gs_user from {$_ENV['eDesDictionary']}gs_chat_user where cd_gs_user=".S::$_User." and cd_gs_chat_room=gs_chat_room.cd_gs_chat_room and cdi_ini<now() and cdi_end is null))
order by last_msg desc, nm_gs_chat_room"
);
while($r=qArray()){
if( $r["type_room"]=="P" ){
qQuery("select email from {$_ENV['eDesDictionary']}gs_user where cd_gs_user in (select cd_gs_user from {$_ENV['eDesDictionary']}gs_chat_user where cd_gs_chat_room=".$r["cd_gs_chat_room"]." and cd_gs_user<>".S::$_User.")", $p1);
...
from {$_ENV['eDesDictionary']}gs_chat_msg_user D, {$_ENV['eDesDictionary']}gs_chat_msg M, {$_ENV['eDesDictionary']}gs_user U, {$_ENV['eDesDictionary']}gs_chat_room R
