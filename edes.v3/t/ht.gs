...
$HFile = '$'.$HFile;
qQuery( "insert into {$_ENV['eDesDictionary']}gs_activity (cd_gs_user, cdi, script, edes, email) values ({$_User}, '{$Cdi}', '{$HFile}', '{$_ENV['ON']}', '".SESS::$_UserEMail."')");
...
if( $_Tipo=='P' ){
include_once( '../_datos/config/desktop.ini' );
$NumCSS = (SESS::$_Desktop==2)?'2':'';
if( file_exists(SESS::$_PathCSS."/principal{$NumCSS}.css") ){
...
if( file_exists(SESS::$_PathCSS."/htm.css" ) ){
...
<?PHP
?>
<BODY onload="Inicio();" onbeforeunload='Salir()' scroll=<?= (($_VerTLF)?'yes':'no') ?> onkeydown='GrabarF10()' onhelp='return false;'<?=((SESS::$_D_=='~')?'':" oncontextmenu='return false;'")?> style='border:0; margin:0; background:#f5f5f5'>
