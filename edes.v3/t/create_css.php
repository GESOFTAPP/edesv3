...
if( preg_match('/(\/all|\/fonts|\/list|\/tab|\/message)/u', $file) || $file=="css/desktop.css" ){
$txt = '(storage)';
if( qCount("{$_ENV['eDesDictionary']}gs_storage", "type_storage='c' and key_storage='{$file}'")==0 ){
qQuery("insert into {$_ENV['eDesDictionary']}gs_storage (type_storage,key_storage,cdi,text_es) values ('c','{$file}','{$cdi}','{$txt}')");
}else{
qQuery("update {$_ENV['eDesDictionary']}gs_storage set cdi='{$cdi}', text_es='{$txt}' where type_storage='c' and key_storage='{$file}'");
}
}
}
...
qQuery("select * from {$_ENV['eDesDictionary']}gs_storage {$where} order by cdi");
while($r=qArray()){
if( $r['type_storage']=="c" ){
