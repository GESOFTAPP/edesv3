...
if( SESS::$_gsACCESO['ACCESO'] < 1 ) die('Error:78');
if( SESS::$_gsACCESO['Edit'] < 1 ) die('Error:79');
...
$Todo = fread( $fd, filesize($Fichero) );
fclose($fd);
if( SESS::$_gsACCESO['LOGEAR'] > 0 ) gsLogear( 'PHP','L', $Fichero.' '.mb_strlen($Todo) );
$Todo = str_replace( '&nbsp;','&nbsp;', $Todo );
$Todo = str_replace( '&#46;','&#46;', $Todo );
$Todo = str_replace( '&#44;','&#44;', $Todo );
$Todo = str_replace( '&#35;','&#35;', $Todo );
...
if( SESS::$_gsACCESO['LOGEAR'] > 0 ) gsLogear('PHP','I', $Fichero.' '.filesize($Fichero));
eHTML();
echo '<SCRIPT type="text/javascript">';
...
$oDoc = '';
if( SESS::$_gsACCESO['LOGEAR'] > 0 ) gsLogear( 'ED','W', $xFichero.' '.mb_strlen($Doc).'/'.(mb_strlen($Doc)-filesize($xFichero) ) );
gsActivity( $xFichero );
$df = fopen($xFichero,'r');
...
if( SESS::$_gsACCESO['TIPO'] != '~' && $Fichero[0] == '$' ) die('<SCRIPT type="text/javascript">top.IE.Quit();</SCRIPT>');
...
fclose( $pnt );
clearstatcache();
if( SESS::$_gsACCESO['LOGEAR'] > 0 ) gsLogear( 'ED','W', $Fichero.' '.mb_strlen($Doc).'/'.(mb_strlen($Doc)-$Long) );
gsActivity( $Fichero );
if( SESS::$_gsACCESO['FTP'] >= 2 ){
if( isset($FTP) ){
...
if( mb_substr($File,0,7+mb_strlen(SESS::$_PathCSS)) == "/http/".SESS::$_PathCSS."/" && eSubstrCount('~AM',$_TipoUsu)==0 ) $Editar = 0;
...
if( SESS::$_gsACCESO['LOGEAR'] > 0 ) gsLogear( 'ED','L', $File );
if( $OriFile[0] == '$' && SESS::$_gsACCESO['TIPO'] != '~' ){
$Editar = 0;
}
}
$Debug = 0;
...
if( SESS::$_gsACCESO['LOGEAR'] > 0 ) gsLogear( 'ED','W', $File.':'.$TABLA.' '.mb_strlen($Todo).'/-'.mb_strlen($Todo) );
}
...
<?PHP
if( SESS::$_Development || SESS::$_D_!='' ) echo "<SCRIPT type='text/javascript' SRC='".gsJS('desgrana')."'></SCRIPT>";
...
if( mb_substr($tabla[$n],0,6)=='outer ' ) $tabla[$n] = mb_substr($tabla[$n],6);
