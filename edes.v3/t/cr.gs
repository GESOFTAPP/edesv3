...
die('En "/edes.v3/t/install_key.php" las variables "$loginInstall" y "$passwordInstall" están comentadas.'."\n");
}
$__Enter = "\n";
$_eDesTitle = "";
...
if( PHP_SAPI === "cli" ) echo "\033[H\033[2J";
printStr("Procesando...");
class SESS {
public static $_D_ = "A";
public static $_Development = true;
public static $spider = ['opcion'=>''];
...
$charset = 'UTF-8';
$lng = "";
if( !empty(SESS::$_LANGUAGE_) ) $lng = SESS::$_LANGUAGE_;
if( empty($lng) && !empty($_SESSION["e-language"]) ) $lng = $_SESSION["e-language"];
if( empty($lng) && !empty(SESS::$_LanguageDefault) ) $lng = SESS::$_LanguageDefault;
...
echo '<span style="font-family:monospace; font-size:12px; color:#00009C;background:#D6DFE7;"><B>:'.$_gsTRACE[$i].'</B></span><br>';
}
}
}
}
...
include($fileIni);
if( $DBDrive!="mysqli" ){
die("Controlador {$DBDrive} no implementado\n");
}
$_ENV['HttpHostName'] = $HttpHostName;
$_ENV['DBSysDictionary'] = $DBSysDictionary ?? "";
if( !empty($_ENV['DBSysDictionary']) ){
$_ENV['DBSysDictionary'] .= ".";
}
$_ENV['eDesDictionary'] = $_ENV['DBSysDictionary'];
if( defined('STDIN') ){
...
if( SESS::$_UserLogin=="-NO-" ){
if( $_POST['context']!=eContextPK() ){
die("error\n");
}
$Pass = mb_strtoupper(md5($_POST["Pass"]));
SESS::$tmp['_PSDV'] = $Pass;
SESS::$_UserLogin = $_POST["Login"];
SESS::$_gsACCESO = array();
$_Web_ = SESS::$_WEB_;
}
$_DirG = 'g/e';
if( SESS::$_gsACCESO['TIPO']=="" ){
...
SESS::$_D_ = '~';
$_ENV[SYS]['context'] = 10;
SESS::$_gsNomUser = $GLOBALS['_gsNomUser'];
}else{
$GLOBALS['_gsNomUser'] = SESS::$_gsNomUser;
}
if( SESS::$_gsACCESO['TIPO']=='~' ){
SESS::$_D_ = '~';
}
if( SESS::$_gsACCESO['ACCESO']<1 ){
}
if( SESS::$_gsACCESO['Create']<1 ){
}
$_Web_ = SESS::$_WEB_;
$_ModoEDES = array('C'=>'c','M'=>'m','I'=>'a','B'=>'b','L'=>'l');
$_DimCol = explode(',','TIPO,LOGIN,CLAVE,ACCESO,EMAIL,iTools,Edit,Help,Shell,Tools,Tree,Icon,Create');
...
<?PHP  } ?>
<TR><TH>PROYECTO
<?PHP  if( SESS::$_gsACCESO['Create'] > 1 ) echo "<TR><TD E='01234' o=CW>Crear"; ?>
...
<?PHP  if( SESS::$_gsACCESO['Create'] > 2 ) echo "<TR><TD E='23' o=BW SiNo SiNo2 disabled>Borrar"; ?>
...
<?PHP if( SESS::$_gsACCESO['Create'] > 1 ){ ?>
<TR><TD E='' style="font-size:1px; vertical-align:middle; background:#9fa3a5"></TR>
...
<?PHP } ?>
<TR><TD E='' style="font-size:1px; vertical-align:middle; background:#9fa3a5"></TR>
<?PHP  if( SESS::$_gsACCESO['Create'] > 1 ) echo "<TR><TD E='23' disabled o=MC id=CCMM><B>Crear INTRANET</B>"; ?>
...
if( !isset($_ENV["DirWeb"]) && PHP_SAPI != "cli" ){
echo '<table width="100%" border=1px cellspacing=0px cellpadding=0px>';
}
TestDB($DBDrive, $DBHostName, $DBUser, $DBPassword, $DBDictionary, $DBPort);
if( $_opCreaDirBase ){
...
if( $CreateDatabase ){
$_HndDB = CrearBD($DBDrive, $DBHostName, $DBUser, $DBPassword, $DBDictionary, $DBPort);
}else{
$_HndDB = ConectarBD($_Sql, $_SqlHostName, $_SqlUsuario, $_SqlPassword, $DBPort);
}
if( $CreateTables ){
$_HndDB = '';
include('edes.v3/'.$DBDrive.'.inc');
if( empty($_HndDB) ){
sql_Conecta();
}
SESS::$sql['driver'] = $DBDrive;
...
printStr('Grabado usuario de desarrollo', "<TR><TH colspan=3 id=FIN class=GRUPO style='text-align:center'>@</TH>");
printStr("INTRANET CREADA EN \"{$_Web_}\"\n", "<TR><TH colspan=3 id=FIN class=GRUPO style='text-align:center; font-size:150%; padding:12px;'>@</TH></TABLE>");
if( PHP_SAPI === "cli" ){
chdir("../../");
echo "Estableciendo usuario y grupo a: www-data:www-data\n\n";
system("chown -R www-data:www-data {$_Web_}");
exit;
}
global $_InstallWithUrl;
if( empty($_InstallWithUrl) ){
echo '<script type="text/javascript">';
echo 'top.SetMenus(3);';
echo 'document.body.scrollTop = 999999;';
echo '</SCRIPT>';
}else{
echo '<script type="text/javascript">';
echo 'setTimeout(function(){ document.body.scrollTop = 999999; }, 1000);';
echo '</SCRIPT>';
}
die('</body></html>');
}
...
global $_NErrores;
$_NErrores++;
if( $_NErrores==1 && PHP_SAPI != "cli" ){
echo '<BR><FONT COLOR="red"><B><U>ERRORES</U></B></FONT><br><BR>';
}
printStr($txt.'<br>');
}
function InitHTML(){
...
function TestDB($_Sql, $_SqlHostName, $_SqlUsuario, $_SqlPassword, $_SqlDiccionario, $DBPort){
global $_DEBUG;
$Pass = $_SqlPassword;
if( mb_strlen($Pass)>8 && mb_substr($Pass,0,8)==dechex(crc32(mb_substr($Pass,8))) ){
$Pass = gzuncompress(base64_decode(mb_substr($Pass,8)));
}
if( eSqlType('mysql') ){
$_HndDB = mysqli_connect($_SqlHostName, $_SqlUsuario, $Pass);
if( !$_HndDB ){
if( PHP_SAPI === "cli" ){
printStr("ERROR al conectar con DB");
if( mysqli_error()!='' ) printStr(mysqli_error());
echo "Host: {$_SqlHostName}, Usuario: {$_SqlUsuario}, Password: {$Pass}";
}else{
echo "<TR><TD id=E>ERROR al conectar con DB";
if( mysqli_error()!='' ) echo "<TR><TD id=E>".mysqli_error();
echo "<TR><TD id=E>Host: <B>{$_SqlHostName}</B>, Usuario: <B>{$_SqlUsuario}</B>, Password: <B>{$Pass}</B>";
...
$_HndDB = mysqli_connect($_SqlHostName, $_SqlUsuario, $Pass, $_SqlDiccionario, $DBPort);
}
if( !$_HndDB ){
if( PHP_SAPI === "cli" ){
printStr("ERROR al conectar con DB");
if( mysqli_error()!='' ) printStr(mysqli_error());
echo "Host: {$_SqlHostName}, Usuario: {$_SqlUsuario}, Password: {$_SqlPassword}";
}else{
echo "<TR><TD id=E>ERROR al conectar con DB";
if( mysqli_error()!='' ) echo "<TR><TD id=E>".mysqli_error();
...
if( PHP_SAPI === "cli" ){
printStr("ERROR al conectar con DB");
printStr(trim(ifx_error()).' : '.trim(ifx_errormsg($_HndDB)));
echo "Host: {$aux}, Usuario: {$_SqlUsuario}, Password: {$_SqlPassword}";
}else{
echo "<TR><TD id=E>ERROR al conectar con DB";
...
function CrearBD($_Sql, $_SqlHostName, $_SqlUsuario, $_SqlPassword, $_SqlDiccionario, $DBPort){
global $_HndDB;
printStr($_SqlDiccionario, '<TR><TH colspan=3 id=c class=GRUPO>CREANDO BASE DE DATOS "@"</TH>');
...
printStr($_SqlHostName.' - '.$_SqlUsuario.' - '.$Pass.' - '.$DBPort, "<TR><TD id=0 colspan=3>Conectando como: @");
if( empty($DBPort) ){
$_HndDB = mysqli_connect($_SqlHostName, $_SqlUsuario, $Pass, "");
}else{
$_HndDB = mysqli_connect($_SqlHostName, $_SqlUsuario, $Pass, "", $DBPort);
}
if( !$_HndDB ){
printStr(mysqli_error(), "<TR><TD id=E colspan=3>ERROR al conectar con DB - @");
exit;
}
mysqli_query($_HndDB, "SET SESSION sql_mode='ALLOW_INVALID_DATES,IGNORE_SPACE,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION'");
if( !empty($_ENV['DBSysDictionary']) ){
$SysDatabase = substr($_ENV['DBSysDictionary'], 0, -1);
$result = mysqli_query($_HndDB, "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME='{$SysDatabase}'");
$r = mysqli_fetch_row($result);
if( $r[0]!=$SysDatabase ){
printStr("Creando diccionario: ".$SysDatabase, "<TR><TD id=0 colspan=3>Creando diccionario: @");
mysqli_query($_HndDB, 'CREATE DATABASE '.$SysDatabase);
}else{
printStr("Borrando diccionario: ".$SysDatabase, "<TR><TD id=0 colspan=3>Borrando diccionario: @");
mysqli_query($_HndDB, 'DROP DATABASE '.$SysDatabase);
$result = mysqli_query($_HndDB, "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME='{$SysDatabase}'");
$r = mysqli_fetch_row($result);
if( $r[0]==$SysDatabase ){
printStr("ERROR no se ha podido borrar el DataBase ".$SysDatabase, '<TR><TD id=E colspan=3>ERROR no se ha podido borrar el DataBase "@"');
exit;
}
printStr("Creando diccionario: ".$SysDatabase, "<TR><TD id=0 colspan=3>Creando diccionario: @");
mysqli_query($_HndDB, 'CREATE DATABASE '.$SysDatabase);
...
$result = mysqli_query($_HndDB, "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME='{$_SqlDiccionario}'");
$r = mysqli_fetch_row($result);
if( $r[0]!=$_SqlDiccionario ){
printStr("Creando diccionario: ".$_SqlDiccionario, "<TR><TD id=0 colspan=3>Creando diccionario: @");
mysqli_query($_HndDB, 'CREATE DATABASE '.$_SqlDiccionario);
}else{
printStr("Borrando diccionario: ".$_SqlDiccionario, "<TR><TD id=0 colspan=3>Borrando diccionario: @");
mysqli_query($_HndDB, 'DROP DATABASE '.$_SqlDiccionario);
$result = mysqli_query($_HndDB, "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME='{$_SqlDiccionario}'");
$r = mysqli_fetch_row($result);
if( $r[0]==$_SqlDiccionario ){
printStr("ERROR no se ha podido borrar el DataBase ".$_SqlDiccionario, '<TR><TD id=E colspan=3>ERROR no se ha podido borrar el DataBase "@"');
exit;
}
printStr("Creando diccionario: ".$_SqlDiccionario, "<TR><TD id=0 colspan=3>Creando diccionario: @");
mysqli_query($_HndDB, 'CREATE DATABASE '.$_SqlDiccionario);
...
for( $i=0; count($Dim); $i++) printStr($Dim[$i], '<br>.@');
die('Error: Sin mecanizar');
}else if( eSqlType('oracle') ){
printStr('ERROR Cuando la "Base de datos" es Oracle tiene que estar creada y la opción desactivada', '<TR><TD id=E colspan=3>@');
exit;
}
return $_HndDB;
}
...
echo '<caption>Definición '.$_Sql.'</caption>';
}
}
for($i=0;$i<count($TIPO);$i++){
if( $vt && PHP_SAPI != "cli" ) echo '<tr>';
$TIPO[$i][0] = mb_strtolower( $TIPO[$i][0] );
for($n=0;$n<count($TIPO[$i]);$n++){
$TIPO[$i][$n] = trim( $TIPO[$i][$n] );
if( $vt ) printStr($TIPO[$i][$n], '<td>@');
}
$_Tipos[mb_strtoupper($TIPO[$i][0])] = array( $TIPO[$i][1], $TIPO[$i][2], $TIPO[$i][3]);
}
if( $vt && PHP_SAPI != "cli" ) echo '</table><br>';
}
function CrearDirBase($NomDir, $PathDir){
printStr("CREANDO DIRECTORIOS", "<TR><TH colspan=3 id=c class=GRUPO>@</TH>");
$dir = $NomDir.'/';
CreaDir( $dir );
printStr($dir, "<TR><TD colspan=3 id=0>@</TD>");
...
printStr("COPIANDO FICHEROS", "<TR><TH colspan=3 id=c class=GRUPO>@</TH>");
...
$_Nivel++;
if( $Rastro ) printStr("[$dorg -> $ddest]", "<BR><BR>@");
if( !is_readable(  $dorg ) ) printStrExit("Error al abrir el directorio de origen '$dorg'", "<br>@" );
if( !is_writeable( $dorg ) ) printStrExit("Error al abrir el directorio de origen '$dorg'", "<br>@" );
if( !is_readable(  $ddest ) ) printStrExit("Error al abrir el directorio de destino '$ddest'", "<br>@" );
if( !is_writeable( $ddest ) ) printStrExit("Error al abrir el directorio de destino '$ddest'", "<br>@" );
$di = opendir( $dorg );
while( $file = readdir( $di ) ){
if( ($file != '.') && ($file != '..') ){
if( file_exists($dorg.$file) != 1 ) printStrExit(" >>>>>>>>>>>> No existe el fichero [".$file."]", "<br>@");
...
printStr($ddest.$file, "<TR><TD id=0 colspan=3>@");
if( !copy( "{$dorg}{$file}", "{$ddest}{$file}" ) ) printStr(' -> ERROR: '."{$dorg}{$file} -> {$ddest}{$file}");
...
closedir( $di );
if( $Rastro&& PHP_SAPI != "cli" ) echo '<BR>';
$_Nivel--;
}
...
if( $file!='.' && $file!='..' ){
if( is_dir($dorg.'/'.$file) ){
if( $Rastro ) echo "D: {$dorg}/{$file}<BR>\n";
BorrarFicheros("$dorg/$file", $ext);
}else if(  eSubstrCount($file, ".{$ext}") ){
if( $Rastro ) echo "Delete: {$dorg}/{$file}<br>\n";
unlink("{$dorg}/{$file}");
}
}
}
...
printStr('CREANDO "empty_page.htm"', '<TR><TH colspan=3 id=c class=GRUPO>@</TH>');
...
printStr('CREANDO "empty_frame.htm"', '<TR><TH colspan=3 id=c class=GRUPO>@</TH>');
...
printStr('CREANDO "empty_html.htm"', '<TR><TH colspan=3 id=c class=GRUPO>@</TH>');
...
printStr('CREANDO "shadown.htm"', '<TR><TH colspan=3 id=c class=GRUPO>@</TH>');
...
printStr('CREANDO "index.htm"', '<TR><TH colspan=3 id=c class=GRUPO>@</TH>');
...
printStr('CREANDO "group.var"', '<TR><TH colspan=3 id=c class=GRUPO>@</TH>');
$fileIni = $_DirDestino.'/_doc_/install.ini';
if( !empty($_ENV['install_ini']) ){
$fileIni = "edes.v3/{$_ENV['install_ini']}";
}
...
printStr("INSERTANDO REGISTROS", '<TR><TH colspan=3 id=c class=GRUPO>@</TH></TR>');
if( eSqlType('mysql,mysqli') ){
$Hoy = date('Y-m-d');
}else{
$Hoy = date('d-m-Y');
}
if( qCount("{$_ENV['eDesDictionary']}gs_tree", '')>0 ){
qQuery("delete from {$_ENV['eDesDictionary']}gs_tree");
}
if( qCount("{$_ENV['eDesDictionary']}gs_tree", '')==0 ){
printStr("ARBOL: Master", "<TR><TD colspan=3>@</TD>");
sql_Inserta( "{$_ENV['eDesDictionary']}gs_tree",
'nm_gs_tree, cd_tree , filename   ,   permission   , extract, excel,pdf,csv,warnings,print,email,news',
"'MASTER'  , 'MASTER','master.txt', '{$_ENV['ON']}',   '{$_ENV['ON']}'  ,  '{$_ENV['ON']}' ,'{$_ENV['ON']}','{$_ENV['ON']}',   0   , '{$_ENV['ON']}' , '{$_ENV['ON']}' , '{$_ENV['ON']}'" );
}
if( qCount("{$_ENV['eDesDictionary']}gs_node", '')>0 ){
qQuery("delete from {$_ENV['eDesDictionary']}gs_node");
}
if( qCount("{$_ENV['eDesDictionary']}gs_node", '')==0 ){
printStr("NODO: Master", "<TR><TD colspan=3>@</TD>");
sql_Inserta( "{$_ENV['eDesDictionary']}gs_node",
'   permission  , nm_gs_node,  dt_add',
"'{$_ENV['ON']}',  'MASTER' , '{$Hoy}'" );
}
if( qCount("{$_ENV['eDesDictionary']}gs_user", '')>0 ){
qQuery("delete from {$_ENV['eDesDictionary']}gs_user");
}
if( qCount("{$_ENV['eDesDictionary']}gs_user", '')==0 ){
if( $LoginType==1 ){
$uLogin = $MasterEMail;
...
echo "<TR><TD>USUARIO:<TD align=center>Login:<BR><B>{$uLogin}</B> <TD align=center> Password:<BR><B>".$DefPassword."</B></TD>";
}
sql_Inserta( "{$_ENV['eDesDictionary']}gs_user",
"dni, login, email, pass, permission,cd_gs_node, cd_gs_tree,  user_name, user_surname, webmaster,export_level,  dt_add , desktop_type,           ys_news        , system_user, cd_type_tree",
"'-',  '-' ,  ''  ,  '-',     ''    ,     0    ,      0    ,'(INTERNAL',   'SYSTEM)' ,    ''    ,      9     , '{$Hoy}',      -1     , '".date('Y-m-d H:i:s')."',     '{$_ENV['ON']}'    ,      ''     " );
sql_Inserta( "{$_ENV['eDesDictionary']}gs_user",
"dni       ,    login  ,     email      ,      pass       , permission,cd_gs_node, cd_gs_tree,      user_name     , user_surname,   webmaster   ,export_level,  dt_add , desktop_type,           ys_news        ,   system_user  , cd_type_tree,   log_user    ,  log_history  ,print_tab_public,  print_public  ,  pdf_public   ,   xls_public  ,  xml_public   ,  txt_public   ,  csv_public   , cd_gs_language",
"'99999901','{$uLogin}','{$MasterEMail}','{$sDefPassword}',     'S'   ,     1    ,      1    , '{$MasterUserName}',     ''      ,'{$_ENV['ON']}',      9     , '{$Hoy}',      -1     , '".date('Y-m-d H:i:s')."', '{$_ENV['ON']}',      'P'    ,'{$_ENV['ON']}','{$_ENV['ON']}', '{$_ENV['ON']}', '{$_ENV['ON']}','{$_ENV['ON']}','{$_ENV['ON']}','{$_ENV['ON']}','{$_ENV['ON']}','{$_ENV['ON']}', 	     'es'    " );
sql_Inserta( "{$_ENV['eDesDictionary']}gs_user_tree",
"cd_gs_user, cd_gs_tree,     mode   ",
"    2     ,     1     , 'I,D,V,U,S'");
}
if( qCount("{$_ENV['eDesDictionary']}gs_op", '')>0 ){
qQuery("delete from {$_ENV['eDesDictionary']}gs_op");
}
if( qCount("{$_ENV['eDesDictionary']}gs_op", '')==0 ){
printStr("Opciones: gs_op", "<TR><TD colspan=3>@</TD>");
...
$p = $n+1;
$sql = "insert into {$_ENV['eDesDictionary']}gs_op
...
$pk = $n+1;
qQuery("insert into {$_ENV['eDesDictionary']}gs_tree_op (cd_gs_tree, cd_gs_op) values (1, {$pk})");
}
}
if( qCount("{$_ENV['eDesDictionary']}gs_toperacion", "")>0 ){
qQuery("delete from {$_ENV['eDesDictionary']}gs_toperacion");
}
if( qCount("{$_ENV['eDesDictionary']}gs_toperacion","")==0 ){
printStr("GS_TOPERACION:", "<TR><TD colspan=3>@</TD>");
for($n=0; $n<count($Dim); $n++){
sql_Inserta("{$_ENV['eDesDictionary']}gs_toperacion",
'cd_gs_toperacion, nm_gs_toperacion, orden, grupo, activa',
"'".$Dim[$n][0]."', '".$Dim[$n][1]."', ".($n+1).", '', '{$_ENV['ON']}'"
);
}
}
if( qCount("{$_ENV['eDesDictionary']}gs_icon", "")>0 ){
qQuery("delete from {$_ENV['eDesDictionary']}gs_icon");
}
$tReg = _ImportTable("{$_ENV['eDesDictionary']}gs_icon"	 , "edes.v3/web/edesweb/gs_icon.unl");
printStr('gs_icon: '.number_format($tReg, 0, ",", "."), '<tr><TD colspan=3>gs_icon: @</td>');
if( qCount("{$_ENV['eDesDictionary']}gs_storage", "")>0 ){
qQuery("delete from {$_ENV['eDesDictionary']}gs_storage");
}
$tReg = _ImportTable("{$_ENV['eDesDictionary']}gs_storage", "edes.v3/web/edesweb/gs_storage.unl");
printStr('gs_storage: '.number_format($tReg, 0, ",", "."), '<tr><TD colspan=3>@</td>');
}
...
printStr("CREANDO TABLAS DEL SISTEMA", "<TR><TH colspan=3 id=c class=GRUPO>@</TH>");
...
clearstatcache();
$_opCreaArbol = true;
printStr("\nCREANDO TABLAS DE LA APLICACIÓN", "<TR><TH colspan=3 id=c class=GRUPO>@</TH>");
$fileSql = $DirDestino.'/_doc_/tbl/_install_es.sql';
if( !empty($_ENV['install_sql']) ){
$fileSql = "edes.v3/{$_ENV['install_sql']}";
}
CrearFCH($DirDestino, $fileSql, '', true, true, false);
printStr("\nCREANDO TABLAS DEL ALMACEN", "<TR><TH colspan=3 id=c class=GRUPO>@</TH>");
...
global $_Importar;
printStr('CARGANDO TABLAS AUXILIARES', "<TR><TH colspan=3 id=c class=GRUPO>@</TH>");
...
echo '<tr><td>'.$_Importar[$i][0].'.ent<td align=right style="padding-right:10px">'.eNumberFormat($Treg).'<td>&nbsp;'.trim($NomTabla);
}
$aReg = 0;
while( !feof($fd) ){
$x = trim(fgets($fd, 2048));
...
qQuery("SHOW TABLES FROM {$DBDictionary}");
while($row = qRow()){
$DimTabla[$row[0]] = true;
}
...
printStr($NomTabla, "<TR><TD colspan=2>@");
$TReg = 0;
...
if( $Ubicado=='/' ) $Ubicado = '';
printStr("Menú: ".$NomMenu, '<TR><TD colspan=3 class="TableName">@');
...
if( PHP_SAPI != "cli" ) printStr($NomEntidad, '<TR><TD class="TableName">@');
...
if( PHP_SAPI != "cli" ) echo '<TD class="NameEDF">&nbsp;';
if( $ConFCH && !$FileStore ){
$NomDir = $_DirFCH.$xDir;
if( !is_dir( $NomDir ) ){
...
if( PHP_SAPI != "cli" ) echo '<TD class="NameTable3">&nbsp;';
if( $_opCreaTablas ){
if( ExisteTabla($NomTabla) ){
qQuery('drop table '.$NomTabla);
}
echo $NomTabla."\n";
...
printStr($txt, '<TR><TD colspan=3 style="background:#CCFFFF">@');
qQuery($txt);
}
}
fclose($ar);
...
try {
if( !($_HndDB instanceof mysqli) || $_HndDB->connect_errno ){
throw new Exception("Conexión a la base de datos no válida");
}
$tabla = $_HndDB->real_escape_string($tabla);
$result = $_HndDB->query("SHOW FIELDS FROM `{$tabla}`");
if ($result === false) {
}
}catch(Exception $e){
...
@unlink($DirNuevo.'/gs_storage.unl');
@unlink($DirNuevo.'/_tmp/__tron.-1');
@unlink($DirNuevo.'/_tmp/__tron.-1.bak');
if( isset($_ENV["DirWeb"]) ){
printStr($DirNuevo, '<TR><TH colspan=3 id=c class=GRUPO>CREANDO WEB "@"</TH>');
return;
}
if( PHP_SAPI === "cli" ){
...
<SCRIPT type="text/javascript">
try{
top.S.ok('Nueva web creada en "<b><?= $DirNuevo ?></b>"');
}catch(e){}
try{
...
$gsEdition = ((SESS::$_D_=='~') ? ' onload="top.eITools(window);"':'');
...
</HEAD><BODY<?= $gsEdition; ?> on-scroll=S(document.body).scrollSet("#DATOS"); onhelp='return top.gsHelp("$dv_usuarios",event);'<?=( (SESS::$_D_!='~') ? ' oncontextmenu="return false;"' : '' )?> onselectstart='return false;'>
...
$_SqlPassword = $DBPassword;
$_SqlDiccionario = $DBDictionary;
if( eSqlType('oracle') ) $_SqlUsuario = mb_strtoupper($_SqlUsuario);
...
if( PHP_SAPI === "cli" ){
echo "¿El directorio ya existe?. ¿Quieres borrarlo? (y/n): ";
...
$TReg++;
qQuery("insert into {$table} values ({$txt})");
}
fclose($fd);
return $TReg;
}
...
$tab = "    ";
$enter = ( PHP_SAPI === "cli" ) ? "\n" : "<br>";
echo "{$enter}Datos de conexión:{$enter}";
echo "{$tab}Host........: {$host}{$enter}";
echo "{$tab}DataBase....: {$dbname}{$enter}";
echo "{$tab}Port........: {$port}{$enter}";
echo "{$tab}User........: {$user}{$enter}";
echo "{$tab}PasswordHost: {$pass}{$enter}{$enter}";
try{
echo "1/14. Intentando conectar al servidor MySQLi...{$enter}";
$conn = $port
? new mysqli($host, $user, $pass, '', $port)
: new mysqli($host, $user, $pass);
if( !$conn || $conn->connect_error ){
throw new Exception("{$tab}Error de conexión: " . $conn->connect_error);
}
echo "{$tab}✓ Conexión al servidor MySQLi exitosa{$enter}";
echo "{$tab}  Conjunto de caracteres inicial: " . $conn->character_set_name() . "{$enter}";
}catch(Exception $e){
die("{$tab}✗ Fallo en conexión: " . $e->getMessage());
}
try{
echo "{$enter}2/14. Comprobando la existencia de la DDBB '{$dbname}'...{$enter}";
$result = $conn->query("SELECT SCHEMA_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = '{$dbname}'");
if( $result->num_rows > 0 ){
if( PHP_SAPI === "cli" ){
$tab = "     ";
echo "{$tab}¿Borrar la DDBB '{$dbname}'?. ¿Quieres borrarla? (y/n): ";
$respuesta = (defined('STDIN') ? trim(fgets(STDIN)) : '');
if( $respuesta === 'y' ){
if( !$conn->query("DROP DATABASE IF EXISTS `{$dbname}`") ){
throw new Exception("{$tab}Error al borrar DDBB: " . $conn->error);
}
}else{
die("{$tab}✗ La DDBB ya existe{$enter}");
}
}else{
if( !$conn->query("DROP DATABASE IF EXISTS `{$dbname}`") ){
throw new Exception("{$tab}Error al borrar DDBB: " . $conn->error);
...
die("{$tab}✗ Error al comprobar si existe la DDBB: " . $e->getMessage());
}
try{
echo "{$enter}3/14. Intentando crear DDBB '{$dbname}'...{$enter}";
$sql = "CREATE DATABASE IF NOT EXISTS `{$dbname}`
CHARACTER SET utf8mb4
COLLATE utf8mb4_unicode_ci";
if (!$conn->query($sql)) {
throw new Exception("{$tab}Error al crear DDBB: " . $conn->error);
}
echo "{$tab}✓ DDBB creada correctamente (UTF-8mb4){$enter}";
}catch(Exception $e){
die("{$tab}✗ Error al crear DDBB: " . $e->getMessage());
}
try{
echo "{$enter}4/14. Seleccionando DDBB...{$enter}";
if (!$conn->select_db($dbname)) {
throw new Exception("{$tab}Error al seleccionar DDBB: " . $conn->error);
}
echo "{$tab}✓ DDBB seleccionada{$enter}";
}catch(Exception $e){
die("{$tab}✗ Error al seleccionar DDBB: " . $e->getMessage());
}
try{
echo "{$enter}5/14. Configurando charset UTF-8...{$enter}";
if (!$conn->set_charset("utf8mb4")) {
throw new Exception("{$tab}Error al establecer charset: " . $conn->error);
}
echo "{$tab}✓ Charset establecido (UTF-8mb4){$enter}";
echo "{$tab}  Charset actual: " . $conn->character_set_name() . "{$enter}";
}catch(Exception $e){
die("{$tab}✗ Error al configurar charset: " . $e->getMessage());
}
try{
echo "{$enter}6/14. Verificando si existe la tabla 'prueba'...{$enter}";
$result = $conn->query("SELECT TABLE_NAME FROM information_schema.TABLES
WHERE TABLE_SCHEMA = '{$dbname}' AND TABLE_NAME = 'prueba'");
if ($result->num_rows > 0) {
echo "{$tab}✗ La tabla ya existe{$enter}";
}
}catch(Exception $e){
die("✗ Error al verificar si existe la tabla: " . $e->getMessage());
}
try{
echo "{$enter}7/14. Intentando crear tabla `prueba` con codificación UTF-8...{$enter}";
$sql = "CREATE TABLE IF NOT EXISTS `prueba` (
...
if (!$conn->query($sql)) {
throw new Exception("{$tab}Error al crear tabla: " . $conn->error);
}
echo "{$tab}✓ Tabla creada correctamente (UTF-8mb4){$enter}";
$result = $conn->query("SELECT TABLE_COLLATION
FROM information_schema.TABLES
WHERE TABLE_SCHEMA = '{$dbname}'
AND TABLE_NAME = 'prueba'");
if ($result && $row = $result->fetch_assoc()) {
echo "{$tab}Collation de la tabla: " . $row['TABLE_COLLATION'] . "{$enter}";
}
}catch(Exception $e){
die("{$tab}✗ Error al crear tabla: " . $e->getMessage());
}
try{
echo "{$enter}8/14. Intentando insertar datos de prueba...{$enter}";
$sql = "INSERT INTO `prueba` (`nombre`, `email`) VALUES
('Juan Pérez', 'juan@example.com'),
('María García', 'maria@example.com')";
if (!$conn->query($sql)) {
throw new Exception("{$tab}Error al insertar datos: " . $conn->error);
}
echo "{$tab}✓ Datos de prueba insertados correctamente{$enter}";
}catch(Exception $e){
die("{$tab}✗ Error al insertar datos: " . $e->getMessage());
}
try{
echo "{$enter}9/14. Verificando datos insertados...{$enter}";
$result = $conn->query("SELECT COUNT(*) as total FROM `prueba`");
if (!$result) {
throw new Exception("{$tab}Error al verificar datos: " . $conn->error);
}
$row = $result->fetch_assoc();
echo "{$tab}✓ Registros encontrados: " . $row['total'] . "{$enter}";
}catch(Exception $e){
die("{$tab}✗ Error al verificar datos: " . $e->getMessage());
}
try{
echo "{$enter}10/14. Borrando tabla...{$enter}";
if (!$conn->query("DROP TABLE IF EXISTS `prueba`")) {
throw new Exception("{$tab}Error al borrar tabla: " . $conn->error);
}
echo "{$tab}✓ Tabla borrada correctamente{$enter}";
}catch(Exception $e){
die("{$tab}✗ Error al borrar tabla: " . $e->getMessage());
}
try{
echo "{$enter}11/14. Borrando DDBB...{$enter}";
if (!$conn->query("DROP DATABASE IF EXISTS `{$dbname}`")) {
throw new Exception("{$tab}Error al borrar DDBB: " . $conn->error);
}
echo "{$tab}✓ DDBB borrada correctamente{$enter}";
}catch(Exception $e){
die("{$tab}✗ Error al borrar la DDBB: " . $e->getMessage());
}
try{
echo "{$enter}12/14. Verificando que la tabla 'prueba' se ha borrado...{$enter}";
$result = $conn->query("SELECT TABLE_NAME FROM information_schema.TABLES
WHERE TABLE_SCHEMA = '{$dbname}' AND TABLE_NAME = 'prueba'");
if ($result->num_rows > 0) {
echo "{$tab}✗ La tabla no se ha borrado{$enter}";
}
}catch(Exception $e){
die("✗ La tabla no se ha borrado: " . $e->getMessage());
}
try{
echo "{$enter}13/14. Verificando la existencia de la DDBB '{$dbname}'...{$enter}";
$result = $conn->query("SELECT SCHEMA_NAME FROM information_schema.SCHEMATA WHERE SCHEMA_NAME = '{$dbname}'");
if ($result->num_rows > 0) {
die("{$tab}✗ La DDBB no se ha podido borrar{$enter}");
}
}catch(Exception $e){
die("{$tab}✗ Error la DDBB no se ha podido borrar: " . $e->getMessage());
}
try{
$conn->close();
echo "{$enter}14/14. Proceso completado con éxito{$enter}{$enter}";
}catch(Exception $e){
die("{$tab}✗ Error al cerrar la conexión: " . $e->getMessage());
}
}
