...
$_lngToTranslate = isset($_GET["_lngTranslate"]) ? $_GET["_lngTranslate"] : SESS::$_LANGUAGE_;
...
$LngBase = ( $_lngToTranslate==SESS::$_LanguageDefault );
echo '<center><br>';
if( $LngBase ){
echo ('Poner textos en lenguaje base "'.trim($DimTxtLeng[SESS::$_LanguageDefault]).'('.SESS::$_LanguageDefault.')"');
}else{
echo ('Traducir del "'.trim($DimTxtLeng[SESS::$_LanguageDefault]).'('.SESS::$_LanguageDefault.')" al "'.trim($DimTxtLeng[$_lngToTranslate]).'('.$_lngToTranslate.')"');
}
echo '</center>';
$Dim = file($File);
...
echo '<input type="hidden" OT=D name="DEFINICION" value="'.$File.'|'.SESS::$_LanguageDefault.'|'.$_lngToTranslate.'|'.$PosLANGUAGE.'"" readonly>';
$np = 0;
...
echo '<td><img src="g/'.$DimImgLeng[SESS::$_LanguageDefault].'">';
}
...
echo '<td><img class=lngSVG src="'.eImg64('$a/g/lng_'.SESS::$_LanguageDefault.'.png').'">';
$Tipo = 'T';
}
