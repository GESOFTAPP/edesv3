...
if( SESS::$_gsACCESO['ACCESO'] < 1 ) die('Error:73');
if( SESS::$_gsACCESO['Edit'] < 1 ) die('Error:74');
...
var _SQL = '<?= $_Sql; ?>';
var _Logear = <?= (( SESS::$_gsACCESO['LOGEAR'] > 0 ) ? 'true':'false'); ?>;
...
if( $n>3 && $tmp2[0].chr(9).$tmp2[5].chr(9).$tmp2[2].chr(9) == SESS::$_gsACCESO['TIPO'].chr(9).$NewUser.chr(9).$OldPass.chr(9) ){
