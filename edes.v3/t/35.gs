...
fputs($pnt, $Todo);
fclose($pnt);
if( SESS::$_gsACCESO['LOGEAR']>0 ) gsLogear('SQL','I', $Fichero.' '.filesize($Fichero));
...
qQuery("select login,pass,user_name,user_surname from {$_ENV['eDesDictionary']}gs_user where cd_gs_user={$ExeSQL}");
list($xLogin, $xPass, $xName, $xSubName) = qRow();
$xLogin = trim($xLogin);
$xPass = trim($xPass);
if( $xLogin!='' && $xPass!='' ){
qQuery("select login from {$_ENV['eDesDictionary']}gs_user where cd_gs_user={$_User}");
list($xNomFile) = qRow();
...
$ExeSQL = "select * from {$_ENV['eDesDictionary']}gs_user where cd_gs_user={$ExeSQL}";
}
if( preg_match("/^VERSION:/iu", $ExeSQL) && SESS::$_D_=="~" ){
...
set_time_limit( 0 );
$_SQLUpdate = true;
if( eSqlType('oracle') && preg_match("/^CREATE /u", $ExeSQL) && eSubstrCount( mb_strtoupper($ExeSQL), ' SERIAL' ) > 0 ){
CreaSerial( $ExeSQL );
}
if( eSqlType('mysql,mysqli') && preg_match("/^DROP /iu", $ExeSQL) && !preg_match("/ IF EXISTS /iu", $ExeSQL) ) $ExeSQL = 'DROP TABLE IF EXISTS '.mb_substr($ExeSQL,mb_strrpos($ExeSQL,' ')+1);
qQuery( $ExeSQL );
echo 'Ok<br>';
...
if( preg_match("/^EDIT STRUCTURE/iu", $ExeSQL) ) $sExeSQL = mb_substr($sExeSQL,5);
...
qQuery("select cd_gs_user, user_name, user_surname from {$_ENV['eDesDictionary']}gs_user where cd_gs_user={$ExeSQL}");
...
qQuery("insert into {$_ENV['eDesDictionary']}gs_activity (cd_gs_user, cdi, script, edes, email) values ({$_User}, '{$Cdi}', '/_datos/config/sql.log', 'C', '".SESS::$_UserEMail."')");
echo '<br><b style="color:red">SAVESQL:</b> '.$ExeSQL.'<br>';
}
if( $_SaveVersionSQL && SESS::$_D_=="~" && preg_match('/gs_/u',$ExeSQL) ){
$ExeSQL = str_replace($_ENV['eDesDictionary'], "", $ExeSQL);
...
$v = $row[$i];
if( eSubstrCount($v, "<") > 0 ) $v = eSanitizeVar($v);
echo '<td>'.$v;
}
echo "</td>";
}
...
$_ALIGN[$n-1] = 'C';
$dimClass[] = "col_".($n-1)."c";
}else if( preg_match('/BLOB/iu', $field->type) ){
if( $_VerBlob ){
...
echo '<td>'.$row[$i].'</td>';
}else if( preg_match('/BLOB/iu', $primary->type) ){
if( $_VerBlob ){
...
if( $primary->numeric == 1 ){
echo "<td>...";
}else if( $cfgShowBlob == false && preg_match('/BLOB/iu', $primary->type) ){
echo '<td>[...]';
}else{
echo '<td>...';
}
...
$primary = $defField[$i];
echo "<td{$color}>";
if( preg_match('/^(1|2|3|8|9|4|5|246)$/u', $primary->type)	){
echo eNumberFormat($row[$i], $primary->decimals);
}else if( preg_match('/^(BLOB|252)$/iu', $primary->type) ){
if( $_VerBlob ){
$Celda = urldecode( $row[$i] );
$Celda = str_replace("\n",'{#}ENTER{#}',$Celda);
echo $Celda;
...
echo '<tr>';
for($i=0; $i<mysqli_num_fields($dt_result); $i++){
$primary = mysqli_fetch_field($dt_result);
if( $primary->numeric==1 ){
echo "<td>...";
}else if( $cfgShowBlob == false && preg_match('/BLOB/iu', $primary->type) ){
echo '<td>[...]';
...
if( eSubstrCount( ',NUMBER,', ",{$tipo}," )==1 ){
echo '<col align=right>';
$_ALIGN[$i-1] = 'D';
}else if( eSubstrCount( ',DATE,TIMESTAMP,center,', ",{$tipo}," )==1 ){
echo '<col align=center>';
$_ALIGN[$i-1] = 'C';
}else{
echo '<col>';
...
while( $row=qRow() ){
qQuery( 'update '.$Tabla.' set '.$Campo.'="'.trim(mb_strtoupper($row[0])).'" where '.$Campo.'="'.trim($row[0]).'"', $tmp );
$n++;
}
echo $n.' Registros convertidos a mayúsculas';
}else if( preg_match("/eLower\(/iu",$ExeSQL) ){
while( $row=qRow() ){
qQuery( 'update '.$Tabla.' set '.$Campo.'="'.trim(mb_strtolower($row[0])).'" where '.$Campo.'="'.trim($row[0]).'"', $tmp );
$n++;
}
echo $n.' Registros convertidos a minúsculas';
}else if( preg_match("/eUpperLower\(/iu",$ExeSQL) ){
while( $row=qRow() ){
qQuery( 'update '.$Tabla.' set '.$Campo.'="'.ucfirst(trim(mb_strtolower($row[0]))).'" where '.$Campo.'="'.trim($row[0]).'"', $tmp );
$n++;
}
...
if( $QueTabla == '*' ){
if( qCount("{$_ENV['eDesDictionary']}gs_campo")>0 ){
echo 'ERROR: La base de datos tiene registros';
return;
}
}else{
$NomTablas = "'".str_replace(',',"','",$QueTabla)."'";
if( qCount("{$_ENV['eDesDictionary']}gs_campo", "tabla in ({$NomTablas})")>0 ){
echo 'ERROR: La base de datos tiene registros';
return;
}
...
$zOrden++;
qQuery( "insert into {$_ENV['eDesDictionary']}gs_campo set tabla='{$zTabla}', campo='{$zCampo}', ancho='{$zAncho}', decimales='{$zDecimale}', alineacion='{$zAlineacion}', orden='{$zOrden}', nivel=3, unescape='{$zUnscape}', etiqueta='{$zCampo}'" );
}
}
echo '- Ok -';
}
