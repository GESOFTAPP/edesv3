...
SESS::$tmp['_PSDV'] = mb_strtoupper(md5($_POST["Pass"]));
SESS::$_WEB_ = true;
SESS::$_Node = 1;
SESS::$_D_ = '~';
$_ENV[SYS]['context'] = 10;
...
if( SESS::$_gsACCESO['TIPO']=='~' ) SESS::$_D_ = '~';
...
function eLoading(){}
<?PHP  } ?>
<?PHP  if( SESS::$_gsACCESO['TIPO'] == '~' || SESS::$_gsACCESO['TIPO'] == 'A' ){ ?>
function ReInstalar(){
...
<?PHP  if( eSubstrCount('~AM',SESS::$_gsACCESO['TIPO']) == 1 ){ ?>
