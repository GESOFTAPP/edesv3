...
echo "];\n";
$txt = file_get_contents('../_tmp/php/'.SESS::$_G_.'menu.'.$_User).implode(CHR10, $addOptions);
file_put_contents('../_tmp/php/'.SESS::$_G_.'menu.'.$_User, $txt);
...
S('#eIDIOMA','#arSTOOLS',top).text('<?=SESS::$_LANGUAGE_?>');
...
if( $_GET["Development"]=="Tree" ){
$GLOBALS["_AllLanguages"] = qCount("{$_ENV['eDesDictionary']}gs_language", "tf_translation='{$_ENV['ON']}'");
if( $_gsMaster=='~' ){
...
if( eSubstrCount('~AMP', $_gsMaster)==1 ){
RecuperaArbol2('../tree/__personal.'.str_replace(' ', '', mb_strtolower(SESS::$_UserLogin)), $tmp);
}
}
}
