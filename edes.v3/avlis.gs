...
eHTML('$avlis.gs');
?>
<LINK REL="stylesheet" HREF="<?=SESS::$_PathCSS?>/splano.css" TYPE="text/css">
<style>
#o { display: none; }
#c { text-align: center; }
#d { text-align: right; }
...
<?PHP
if( qCount("{$_ENV['eDesDictionary']}gs_exp_file", "cd_gs_user={$_User} and estado='T'" ) == 0 ){
?>
...
if( $MasInfo ){
qSelect("{$_ENV['eDesDictionary']}gs_exp_file", '*', "cd_gs_user={$_User} and cdi>='{$HastaCDI}'", 'cdi DESC' );
}else{
qSelect("{$_ENV['eDesDictionary']}gs_exp_file", '*', "cd_gs_user={$_User} and estado='T' and cdi>='{$HastaCDI}'", 'cdi DESC' );
}
while( $row = qArray() ){
