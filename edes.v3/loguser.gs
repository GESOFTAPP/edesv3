...
if( $_Rastrear ) echo 'SQL: '."select user_name, user_surname from {$_ENV['eDesDictionary']}gs_user where cd_gs_user='{$r[1]}'".'<br>';
qQuery( "select user_name, user_surname from {$_ENV['eDesDictionary']}gs_user where cd_gs_user='{$r[1]}'" );
$r=qRow();
$r[0] = trim($r[0]);
if( $r[0]!='' ){
...
qQuery( "select user_name, user_surname from {$_ENV['eDesDictionary']}gs_user where cd_gs_user='".$r[$nc+1]."'" );
$r2=qRow();
$r2[0] = trim($r2[0]);
if( $r2[0]!='' ){
...
$PorTipo = array();
if( $_Rastrear ) echo 'SQL: '."select * from {$_ENV['eDesDictionary']}gs_log where tabla='{$Tabla}' and clave='{$IndiceLog}' order by 1".'<br>';
qQuery( "select * from {$_ENV['eDesDictionary']}gs_log where tabla='{$Tabla}' and clave='{$IndiceLog}' order by 1" );
while( $rl=qArray() ){
$Fecha = mb_substr($rl['cdi'],0,10);
qQuery( "select user_name, user_surname from {$_ENV['eDesDictionary']}gs_user where cd_gs_user='{$rl['cd_gs_user']}'", $p );
$r=qRow($p);
$r[0] = trim($r[0]);
if( $r[0]!='' ){
$NomUsuario = $r[0].' '.trim($r[1]);
