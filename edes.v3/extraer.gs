...
if( isset($_GET['DB']) ){
if( eSubstrCount(str_replace('\\','/',$_GET['DB']),'/')==0 ) $_GET['DB'] = '/_datos/config/'.$_GET['DB'];
if( eSubstrCount($_GET['DB'],'.')==0 ) $_GET['DB'] .= '.ini';
...
eHTML('$extraer.gs');
?>
<LINK REL='stylesheet' HREF='<?=SESS::$_PathCSS?>/ficha.css' TYPE='text/css'>
</HEAD>
<BODY scroll=auto>
<?PHP
...
$_db_->qQuery("select * from {$_ENV['eDesDictionary']}gs_formato where cd_gs_formato='{$cd_gs_formato}' and cd_gs_user='{$_User}'");
$Formato = $_db_->qArray();
...
$_db_->qQuery( "select * from {$_ENV['eDesDictionary']}gs_entidad where cd_gs_entidad='{$cd_gs_entidad}'" );
$Entidad = $_db_->qArray();
...
eTrace( "SQL...........: select tabla,campo,relacion,etiqueta from {$_ENV['eDesDictionary']}gs_campo where cd_gs_entidad='{$cd_gs_entidad}' and campo in ( $CamposDeCondi )" );
eTrace('');
}
$Busca = '';
if( $CamposDeCondi != '' ){
$TCamposExist = $_db_->qCount("{$_ENV['eDesDictionary']}gs_campo", "cd_gs_entidad='{$cd_gs_entidad}' and campo in ( $CamposDeCondi )" );
$_db_->qQuery( "select tabla,campo,relacion,etiqueta from {$_ENV['eDesDictionary']}gs_campo where cd_gs_entidad='{$cd_gs_entidad}' and campo in ( $CamposDeCondi )" );
...
if( $_db_->qCount( "{$_ENV['eDesDictionary']}gs_formato", "cd_gs_formato='{$cd_gs_formato}' and cd_gs_user='{$_User}'" ) == 0 ){
...
$_db_->qQuery( "select * from {$_ENV['eDesDictionary']}gs_campo where cd_gs_campo={$DimCampos[$n]}" );
...
$_db_->qQuery("insert into {$_ENV['eDesDictionary']}gs_acceso
(cd_gs_toperacion,				 conexion       , objeto, modo,	   edf	   ,      tabla        ,  parametros ,          pagina             , parametro, registros, cd_gs_user, cd_gs_node,    cdi   ) values
(     'DOC'      , '".SESS::$_Connection_."',   'D' , 'E' , '{$NomFile}', '{$cd_gs_destino}', 'extraer,gs','{$Entidad['nm_gs_entidad']}',    ''    , {$tReg} ,  {$_User},  {$_Node} , '{$cdi}' )");
...
$_db_->qQuery("update {$_ENV['eDesDictionary']}gs_exp_file set download='".date('Y-m-d H:i:s')."' where {$FicheroAModificar}");
$_db_->qEnd();
...
if( $GLOBALS['_TronDownLoad'] ){
error_log( "Pendiente....\n", 3, '___download__.info' );
error_log( "insert into {$_ENV['eDesDictionary']}gs_exp_file
...
$_db_->qQuery("insert into {$_ENV['eDesDictionary']}gs_exp_file
(     descripcion  ,   fichero  ,  t_reg  ,    tipo   ,    formato   ,    cd_gs_formato  ,    estado    , comprimido, cd_gs_user ,  sql_1  ,  sql_2  ,  sql_3  ,   cdi  , descargado, sg ) values
('{$gsDescripcion}','{$NomFile}',{$gsTReg},'{$gsTipo}','{$gsFormato}','{$_cd_gs_formato}', '{$gsEstado}',  {$gsZip} ,{$gsUsuario},'$sql[1]','$sql[2]','$sql[3]','{$cdi}',     0     , 0  )");
...
$_db_->qQuery( "select cd_gs_exp_file from {$_ENV['eDesDictionary']}gs_exp_file {$_RegActual}" );
...
$_db_->qQuery( "update {$_ENV['eDesDictionary']}gs_exp_file set estado='{$gsEstado}', t_reg={$gsTReg}, sg={$tSegundos}{$UnaDescarga} where fichero='{$NomFile}' and tipo='{$gsTipo}' and cd_gs_user={$gsUsuario} and cdi='{$gsCdi}'" );
...
$FileCSS = $_DirRoot.SESS::$_PathCSS.'/extraer.css';
$fd = fopen($FileCSS, 'r');
...
$_db_->qQuery( "select * from {$_ENV['eDesDictionary']}gs_campo where cd_gs_entidad='{$cd_gs_entidad}' and campo='nm_".mb_substr($clave,3)."'" );
$r = $_db_->qArray();
if( $r['campo']!='' ){
...
$_db_->qQuery("update {$_ENV['eDesDictionary']}gs_exp_file set t_reg={$Informar} where cd_gs_exp_file={$_IdReg}", $p1);
