...
$_SufijoLanguage = '';
if( qCount("{$_ENV['eDesDictionary']}gs_language", "tf_translation='{$_ENV['ON']}'")>0 ) $_SufijoLanguage = '_'.SESS::$_LANGUAGE_;
if( qCount("{$_ENV['eDesDictionary']}gs_language")==1 ) $_SufijoLanguage = '';
qQuery("select export_level from {$_ENV['eDesDictionary']}gs_user where cd_gs_user='{$_User}'");
list( $ExportLevel ) = qRow();
qQuery("select nm_gs_entidad from {$_ENV['eDesDictionary']}gs_entidad where tabla='{$LabelTabla}'");
$row = qRow();
if( $row[0]!='' ) $LabelTabla = trim($row[0]);
qQuery("delete from {$_ENV['eDesDictionary']}gs_log_tmp where pk_user={$PkUser}");
if( $_DEBUG==1 ) eTrace('---> USER: '.$_User );
$DimVerSiNoSystem = array();
qQuery("select campo, log_history from {$_ENV['eDesDictionary']}gs_campo where tabla='".$_POST["tabla"]."' and log_history<>'N' and log_no_system='{$_ENV['ON']}'");
while( $row=qRow() ){
if( $_DEBUG==1 ) eTrace('---> Log si no es Sistema: '.$row[0] );
$DimVerSiNoSystem[$row[0]] = 'S';
}
$DimLogHistory = array();
qQuery("select campo, log_history from {$_ENV['eDesDictionary']}gs_campo where tabla='".$_POST["tabla"]."' and log_history<>'{$_ENV['ON']}'");
while( $row=qRow() ){
...
$Dim = Array();
$DimCAMPO = array();
$DimTablaLog = array("{$_ENV['eDesDictionary']}gs_log");
$ayo = mb_substr(date('Y'),2);
while( qExists("{$_ENV['eDesDictionary']}gs_log_{$ayo}") ){
$DimTablaLog[] = "{$_ENV['eDesDictionary']}gs_log_{$ayo}";
$ayo--;
}
...
from
{$_ENV['eDesDictionary']}{$TablaLog} L, {$_ENV['eDesDictionary']}gs_user U
...
U.user_surname, U.user_name {$CampoLog}
from
{$_ENV['eDesDictionary']}{$TablaLog} L left join {$_ENV['eDesDictionary']}gs_user U on L.cd_gs_user=U.cd_gs_user
...
qQuery("insert into {$_ENV['eDesDictionary']}gs_log_tmp (pk_user, cdi,operacion,cd_gs_user,tabla,campo,valor) values
...
if( eSqlType('mysql,mysqli') ){
qQuery("select count(distinct(C.cd_gs_grupo)) from {$_ENV['eDesDictionary']}gs_log_tmp L, {$_ENV['eDesDictionary']}gs_campo C where L.pk_user={$PkUser} and L.tabla=C.tabla and L.campo=C.campo and C.log_history<>'N'");
}else{
qQuery("select count(distinct C.cd_gs_grupo)  from {$_ENV['eDesDictionary']}gs_log_tmp L, {$_ENV['eDesDictionary']}gs_campo C where L.pk_user={$PkUser} and L.tabla=C.tabla and L.campo=C.campo and C.log_history<>'N'");
}
...
$Ultimo = '';
$NRepetidos = 0;
qQuery("select * from {$_ENV['eDesDictionary']}gs_log_tmp where pk_user={$PkUser} order by tabla,campo,cdi,valor", $p1);
while( $r=qArray($p1) ){
$OldValor = $r['valor'];
...
$NRepetidos++;
qQuery("update {$_ENV['eDesDictionary']}gs_log_tmp set borrar='{$_ENV['ON']}' where pk_user={$PkUser} and tabla='{$r[tabla]}' and campo='{$r[campo]}' and cdi='{$r[cdi]}'", $p2);
}
$Ultimo = $ValorActual;
}
if( $NRepetidos>0 ) qQuery("delete from {$_ENV['eDesDictionary']}gs_log_tmp where pk_user={$PkUser} and borrar='{$_ENV['ON']}'");
$n = 0;
qQuery("select C.cd_gs_grupo, C.orden, count(*) from {$_ENV['eDesDictionary']}gs_log_tmp L, {$_ENV['eDesDictionary']}gs_campo C where L.pk_user={$PkUser} and L.tabla=C.tabla and L.campo=C.campo and C.log_history<>'N' group by C.cd_gs_grupo, C.orden order by C.cd_gs_grupo, C.orden");
$DimRowSpan = array();
while( $r=qRow() ){
...
{$CampoLog}
from
{$_ENV['eDesDictionary']}gs_log_tmp L,
{$_ENV['eDesDictionary']}gs_user U,
{$_ENV['eDesDictionary']}gs_campo C,
{$_ENV['eDesDictionary']}gs_grupo G
where
...
if( (SESS::$pdf_private==$_ENV['ON'] || SESS::$pdf_public==$_ENV['ON']) && $UrlPDF!='' ){
...
echo '</tr></table>';
echo '</CENTER>';
if( !$VIEW_gs_log_tmp ) qQuery( "delete from {$_ENV['eDesDictionary']}gs_log_tmp where pk_user={$PkUser}" );
?>
<script type="text/javascript">
function Todo(){
