...
if( qCount("{$_ENV['eDesDictionary']}gs_exp_file", "cd_gs_user={$U} and estado!='H' and cdi<'{$HastaCDI}'" ) > 0 ){
qSelect("{$_ENV['eDesDictionary']}gs_exp_file", '*', "cd_gs_user={$U} and estado!='H' and cdi<'{$HastaCDI}'", 'cdi' );
while( $row = qArray() ){
...
sql_Modifica("{$_ENV['eDesDictionary']}gs_exp_file", 'estado="H"', "cd_gs_user={$U} and estado!='H' and cdi<'{$HastaCDI}'" );
}
qCount("{$_ENV['eDesDictionary']}gs_exp_file", "fichero='{$NomFile}' and cdi='{$CdiFile}'" );
qFree();
if( $_TReg != 1 ){
echo '<script type="text/javascript">';
...
sql_Modifica("{$_ENV['eDesDictionary']}gs_exp_file", 'descargado = descargado + 1 ', "fichero='{$NomFile}' and cdi='{$CdiFile}'" );
qSelect("{$_ENV['eDesDictionary']}gs_exp_file", '*', "fichero='{$NomFile}' and cdi='{$CdiFile}'" );
$row = qArray();
if( $row['descargado'] == 1 ){
sql_Modifica("{$_ENV['eDesDictionary']}gs_exp_file", "download='".date('Y-m-d H:i:s')."'", "fichero='{$NomFile}' and cdi='{$CdiFile}'" );
}
qFree();
qEnd();
