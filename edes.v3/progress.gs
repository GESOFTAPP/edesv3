...
qQuery( "select seconds from {$_ENV['eDesDictionary']}gs_progress where script='{$_POST['_SOURCE_']}' and md5='{$Md5}'" );
list( $ns ) = qRow();
$ns = $ns*1;
if( $ns==0 ){
qQuery( "select sum(seconds),count(*) from {$_ENV['eDesDictionary']}gs_progress where script='{$_POST['_SOURCE_']}'" );
list( $ns, $c ) = qRow();
$ns = $ns*1;
