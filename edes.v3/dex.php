...
if( !$_ENV[SETUP]['System']['ContextActivate'] ) return;
if(mb_substr($script,-2)=="()" || mb_substr($script,-3)=="();" ) return;
global $Dir_, $_Sql, $_ContextReadOnly, $_ContextFieldsMD5, $_DBRANGEADD, $_DBSERIAL;
$dim = array();
...
(".SESS::$_Connection_.", {$_ENV[SYS]['context']}, '{$script}', '{$PKSeek}', '{$_ContextReadOnly}', '{$md5fields}', '{$FieldGet}')");
}
...
$txt = $dim[$n];
if( eSubstrCount($txt,'top.eSWOpen(')>0 || eSubstrCount($txt,'eCallSrv(')>0 || eSubstrCount($txt,'location.href')>0 || eSubstrCount($txt,'location.replace(') || eSubstrCount($txt,'eUrl(') ){
$DimCheck[] = $txt;
}
}
...
$f = mb_strpos($txt,';',$i);
$GLOBALS[$pk] = mb_substr($txt,0,$i)."eUrl(".trim(mb_substr($txt, $i2, $f-$i2)).")".mb_substr($txt,$f);
}
...
if($pParentesis>0 && ($pComilla==0 || $pParentesis<$pComilla) ) $pComilla = $pParentesis;
$func = mb_substr($txt,1, $pComilla);
if( eSubstrCount($func,".php")==1 && eSubstrCount($func,"edes.php")==0 && mb_substr($func,0,2)<>"E:" ) $func = "E:".$func;
...
}else if( eSubstrCount($txt,'location.replace(')>0 || eSubstrCount($txt,'eUrl(')>0 ){
...
(".SESS::$_Connection_.", {$_ENV[SYS]['context']}, '{$script}',   ''   ,       ''       ,     ''     )");
}
}
...
for( $n=$desde+1; $n<$t; $n++ ){
if( mb_substr($txt,$n,1)=="'" or mb_substr($txt,$n,1)=='"' ){
return $n;
...
eInit();
die('ERROR: La funciÃ³n "eLngLoad()" no es necesaria en ficheros "DF"');
}
}else if( $File[0]=='*' ){
...
SESS::$_LANGUAGE_ = ($LNG=='') ? $_Language : $LNG;
...
SESS::$SessionMaxLife = date("U")+((($_ENV[SETUP]['System']['SessionMaxLife']!="")? $_ENV[SETUP]['System']['SessionMaxLife']:24)*3600);
...
if( $_ENV['eDesDictionary']!="" && mb_substr($_ENV['eDesDictionary'],-1)!="." ){
$_ENV['eDesDictionary'] .= ".";
...
if( $dexType=="public" ){
qQuery("select login,pass from {$_ENV['eDesDictionary']}gs_user where cd_gs_user={$dexUser}");
...
$UserOk = 0;
$UserDelLogin = 0;
qQuery("select login,pass, pass_tmp, pass_tmp_cdi, cd_gs_user,permission from {$_ENV['eDesDictionary']}gs_user where {$pIzq1}login like {$pIzq2}'".mb_substr($login,0,2)."%'{$pDch2}{$pDch1}");
while( $r=qArray() ){
...
unset(SESS::$_Login_, SESS::$_Password_, SESS::$_Remember_);
unset(SESS::$_PassError_, SESS::$_PassTime_);
}else{
_AtaqueMasivo();
...
if( qCount("{$_ENV['eDesDictionary']}gs_conexion", "cdi>'{$cdiAccess}' and access<=6")>$_LoginNumber ){
eEnd();
}
}
}
...
qQuery("select cd_type_tree,cd_gs_rol,like_user, cd_gs_user from {$_ENV['eDesDictionary']}gs_user where cd_gs_user='{$LikeUser}'", $p1);
$oUsu = qArray( $p1 );
$DimUser[] = $LikeUser;
...
qQuery( "select * from {$_ENV['eDesDictionary']}gs_rol where cd_gs_rol='{$_UserTree}' and permission='{$_ENV['ON']}'" );
$row = qArray();
if( $row['cd_gs_rol']=='' ){
...
$_TypeTree = '';
qSelect("{$_ENV['eDesDictionary']}gs_user U, {$_ENV['eDesDictionary']}gs_tree A",
...
if( SESS::$_PDF_==$_ENV['ON'] && !extension_loaded('pdf') && !$PDFExtension ){
SESS::$_PDF_ = $_ENV['OFF'];
$row['pdf'] = $_ENV['OFF'];
}
}
...
if( !isset(SESS::$_CacheSrv) ) SESS::$_CacheSrv = false;
...
$_Util['print'] = (SESS::$print_tab==$_ENV['ON'] || SESS::$print_list==$_ENV['ON'] || SESS::$print_report==$_ENV['ON']) ? $_ENV['ON'] : $_ENV['OFF'];
if( !isset($_G_) ) $_G_ = '';
...
$sNom_gs_navegador	= SESS::$_Platform_;
$sNombre			= SESS::$_Explorer_;
$sResolucion		= SESS::$_Resolution_;
$sVarios			= "";
if($_TronLogin)	error_log("33a\n", 3, $_TronFile);
if( S::qCount("{$_ENV['eDesDictionary']}gs_navegador", "nm_gs_navegador='{$sNom_gs_navegador}' and nombre='{$sNombre}' and resolucion='{$sResolucion}' and varios='{$sVarios}'")==0 ){
S::qQuery("insert into {$_ENV['eDesDictionary']}gs_navegador
(     nm_gs_navegador  ,     nombre  ,     resolucion  ,     varios) values
('{$sNom_gs_navegador}', '{$sNombre}', '{$sResolucion}', '{$sVarios}')" );
$xNavegador = S::qId();
}else{
S::qQuery("select cd_gs_navegador from {$_ENV['eDesDictionary']}gs_navegador where (nm_gs_navegador='{$sNom_gs_navegador}' and nombre='{$sNombre}' and resolucion='{$sResolucion}' and varios='{$sVarios}')");
$xNavegador = S::qRow();
...
S::qQuery("update {$_ENV['eDesDictionary']}gs_conexion set cd_gs_user='{$_User}', cd_gs_node='{$_Node}', cd_gs_tree='{$_Tree}' where conexion=".SESS::$_Connection_);
...
$sql = "insert into {$_ENV['eDesDictionary']}gs_error (cdi,cd_gs_user,tipo,desde,fichero,linea,texto,trace) values ('{$Cdi}', '{$_User}', 'P', 'PHP', '{$OriFichero}', {$ErrorLinea}, $xTexto, ?)";
ifx_query($sql, $_HndDB, $DimText);
}else{
sql_Inserta("{$_ENV['eDesDictionary']}gs_error", 'cdi,cd_gs_user,tipo,desde,fichero,linea,texto,trace',			  "'{$Cdi}', '{$_User}', 'P', 'PHP', '{$OriFichero}', {$ErrorLinea}, '{$xTexto}', '{$TxtError}'", 'codigo');
}
$Id = qId();
?>
...
if( $_ENV[SYS]['checkSession'] && $_GET['_SESS_']!="" ){
$sg = date("U");
}
...
if( isset(SESS::$SessionMaxLife) && SESS::$SessionMaxLife<date("U") ){
if(S::$__tronEntrar) eTron(date('His: ')."eEnd:19");
eTron("sessiÃ³n borrada: ".$_GET['_SESS_']." | ".SESS::$SessionMaxLife." < ".date("U") );
...
SESS::$zoomBIG = (float)$fBaseBIG / (float)$fBase;
SESS::$zoomTLF = (float)$fBaseTLF / (float)$fBase;
SESS::$zoomSmall = (float)$fBaseSmall / (float)$fBase;
SESS::$factorZoom = $factorZoom;
?>
<style title="all" name="all" type="text/css">
<?PHP
$sufijo = SESS::$cssSufijo;
$txt = file_get_contents(SESS::$_PathCSS."/all{$sufijo}.css");
...
<?PHP include(SESS::$_PathCSS."/tab{$sufijo}.css"); ?>
</style>
<style title="list" name="list" disabled=1 type="text/css">
<?PHP include(SESS::$_PathCSS."/list{$sufijo}.css"); ?>
</style>
<style title="all_big" name="all_big" disabled=1 type="text/css">
<?PHP
$txt = file_get_contents(SESS::$_PathCSS."/all_big.css");
...
"_WebMaster=".((SESS::$_WebMaster==$_ENV['ON'])?'true,':'false,').
"_M_='{$_gsMaster}',".
"_FontNumber='font-family:".$_ENV[SETUP]['CSSDynamic']['FontNumbers']."',";
?>
_BYPHONE = <?= ((SESS::$_BYPHONE) ? 'true' : 'false') ?>,
...
S.setup.language = "_<?=SESS::$_LANGUAGE_?>"
S.exitMaxLife(<?=((SESS::$SessionMaxLife-date("U"))/3600)?>, "La sesiÃ³n caducarÃ¡ en 5 minutos.<br><b>La aplicaciÃ³n se cerrarÃ¡.</b>");
S.marksSetup({
...
formatP4: ["<?=mb_substr($_ENV[SETUP]['System']["_FormatP4EXP"],1,-1)?>", "<?=$_ENV[SETUP]['System']["_FormatP4TKNdb"]?>", "<?=mb_substr($_ENV[SETUP]['System']["_FormatP4EXPdb"],1,-1)?>", "<?=$_ENV[SETUP]['System']["_FormatP4TKNuser"]?>"],
formatF4: ["<?=mb_substr($_ENV[SETUP]['System']["_FormatF4EXP"],1,-1)?>", "<?=$_ENV[SETUP]['System']["_FormatF4TKNdb"]?>", "<?=mb_substr($_ENV[SETUP]['System']["_FormatF4EXPdb"],1,-1)?>", "<?=$_ENV[SETUP]['System']["_FormatF4TKNuser"]?>"],
formatCDI: ["<?=mb_substr($_ENV[SETUP]['System']["_FormatCDIEXP"],1,-1)?>", "<?=$_ENV[SETUP]['System']["_FormatCDITKNdb"]?>", "<?=mb_substr($_ENV[SETUP]['System']["_FormatCDIEXPdb"],1,-1)?>", "<?=$_ENV[SETUP]['System']["_FormatCDITKNuser"]?>"],
formatT: ["<?=mb_substr($_ENV[SETUP]['System']["_FormatTEXP"],1,-1)?>", "<?=$_ENV[SETUP]['System']["_FormatTTKNdb"]?>", "<?=mb_substr($_ENV[SETUP]['System']["_FormatTEXPdb"],1,-1)?>", "<?=$_ENV[SETUP]['System']["_FormatTTKNuser"]?>"],
...
$txt = '';
if( eFileGetVar('Login.HostGet') ) $txt = ", host='".$REMOTE_ADDR."'";
qQuery("update {$_ENV['eDesDictionary']}gs_user set dt_access_last='{$Hoy}'{$txt} where cd_gs_user={$_User}");
?>
<script type="text/javascript" charset="UTF-8">
