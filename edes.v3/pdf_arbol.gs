...
if( $_GET['TipoArbol']=='U' ){
if( $_GET['USER']>0 ){;
qQuery( "select * from {$_ENV['eDesDictionary']}gs_user where cd_gs_user='{$_GET['USER']}'" );
$row = qArray();
$_TITLE = 'ARBOL DE OPCIONES DE "'.trim($row['user_name']).' '.trim($row['user_surname']).'"';
}else{
qQuery( "select * from {$_ENV['eDesDictionary']}gs_user where cd_gs_user='{$_User}'" );
$row = qArray();
...
$nArbol = $_GET['nArbol'];
qQuery("select nm_gs_tree from {$_ENV['eDesDictionary']}gs_tree where cd_gs_tree={$nArbol}");
list( $NomArbol ) = qRow();
$_TITLE = 'ARBOL: '.$NomArbol;
qQuery("select * from {$_ENV['eDesDictionary']}gs_op where cd_gs_op in (select cd_gs_op from {$_ENV['eDesDictionary']}gs_tree_op where cd_gs_tree={$nArbol}) order by seq");
while( $_DimTree[]=qArray() ){}
}else{
exit;
}
...
if( isset( $_PDFPrint ) ){
?>
<HTML><HEAD><LINK REL='stylesheet' HREF='<?=SESS::$_PathCSS?>/lista.css' TYPE='text/css'></HEAD>
...
global $_Color;
if( !file_exists(SESS::$_PathCSS."/pdf.css") ) return;
$Dim = file(SESS::$_PathCSS."/pdf.css");
for( $n=0; $n<count($Dim); $n++ ){
