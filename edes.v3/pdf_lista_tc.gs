...
if( SESS::$pdf_private==$_ENV['OFF'] && SESS::$pdf_public==$_ENV['OFF'] ) eEnd();
set_time_limit(60*5);
...
$nmFile = "pdf.css";
if( isset(SESS::$tmp['VAR_TMP']) ){
if( isset(SESS::$tmp['VAR_TMP']["pdf.css"]) ){
$nmFile = SESS::$tmp['VAR_TMP']["pdf.css"];
unset(SESS::$tmp['VAR_TMP']["pdf.css"]);
}
}
if( !file_exists(SESS::$_PathCSS."/{$nmFile}") ) return;
$Dim = file(SESS::$_PathCSS."/{$nmFile}");
for( $n=0; $n<count($Dim); $n++ ){
