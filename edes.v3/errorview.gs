...
eInclude($_Sql);
qQuery( "select * from {$_ENV['eDesDictionary']}gs_error where codigo={$PK}" );
$row = qArray();
if( $row['codigo']=='' ) exit;
...
<BODY onkeydown='Tecla(event)' onmousewheel='CambiaSlider()' style='margin:0px' onload='Init();' onhelp='return false' oncontextmenu='<?=((SESS::$_D_=='~')?'':'return false')?>' onkeydown='CerrarVentana()'><?PHP
