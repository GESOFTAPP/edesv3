...
qQuery("select * from {$_ENV['eDesDictionary']}gs_form where token='{$Registro}'");
}else{
qQuery("select * from {$_ENV['eDesDictionary']}gs_form where cd_gs_form={$Registro} && cd_gs_node=".SESS::$_Node);
}
$r = qArray();
...
$NextPagina = false;
if( $NextForm!="" ){
qQuery("select * from {$_ENV['eDesDictionary']}gs_form where cd_gs_form={$NextForm} && cd_gs_node=".SESS::$_Node);
$r = qArray();
if( $r["cd_gs_node"]!=SESS::$_Node ){
eMessage('Acceso no autorizado', 'HSE');
}
$dim = explode("|", $r["data"]);
$NextForm = trim($r["next_form"]);
